            ...
            try:
                ...
                for line in output.split("\n"):
                    ...
                    if (
                        ...
                        and "]" in line
                        and "NIF" not in line
                        and "viva_tensor" not in line.lower().replace("viva_tensor_zig:", "")
                    ):
                        data_line = line
                        break
                ...
                if data_line:
                    ...
                    print(
                        f"  {n:5d}x{n}: {mean_gf / 1000:8.2f} {unit}  (±{std_gf:.0f} GFLOPS, n={len(clean)})"
                    )
                else:
...
def print_comparison(pt_results, vt_results):
    ...
    for dtype in ["FP32", "FP16", "INT8"]:
        ...
        print(
            f"│ {'Size':>8s} │ {'viva_tensor':>16s} │ {'PyTorch':>16s} │ {'Delta':>7s} │ {'Winner':>7s} │"
        )
        print(f"├{'─' * 10}┼{'─' * 18}┼{'─' * 18}┼{'─' * 9}┼{'─' * 9}┤")
        ...
        for n in SIZES:
            ...
            if vt and pt:
                ...
                print(
                    f"│ {n:>5d}x{n:<2d} │ {vt_tf:>12.2f} {unit:>3s} │ {pt_tf:>12.2f} {unit:>3s} │ {sign}{delta:>5.1f}% │ {winner:>7s} │"
                )
            elif vt:
                print(
                    f"│ {n:>5d}x{n:<2d} │ {vt['tflops']:>12.2f} {unit:>3s} │ {'N/A':>16s} │ {'':>7s} │ {'':>7s} │"
                )
            elif pt:
                print(
                    f"│ {n:>5d}x{n:<2d} │ {'N/A':>16s} │ {pt['tflops']:>12.2f} {unit:>3s} │ {'':>7s} │ {'':>7s} │"
                )
            else:
