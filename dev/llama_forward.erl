...
profile_w8a16(NumLayers, BlockSize) ->
    ...
    %% Warm-up pass done. Re-time on a fresh layer to get steady-state numbers.
    case length(Layers) > 1 of
        true ->
            io:format("~n--- Warmed-up second pass (layer 1) ---~n"),
            Layer1 = lists:nth(2, Layers),
            #{norm1 := N1b, norm2 := N2b,
              q := Qb, k := Kb, v := Vb, o := Ob,
              gate := Gb, up := Ub, down := Db} = Layer1,
            W0 = UsNow(),
            X1w = rmsnorm(HiddenIn, N1b, ?EPS), W1 = UsNow(),
            X1Fp16w = floats_to_fp16(X1w), W2 = UsNow(),
            Qrw = linear_fp8_w8a16(X1Fp16w, Qb, ?HIDDEN), W3 = UsNow(),
            Krw = linear_fp8_w8a16(X1Fp16w, Kb, ?KV_DIM), W4 = UsNow(),
            Vrw = linear_fp8_w8a16(X1Fp16w, Vb, ?KV_DIM), W5 = UsNow(),
            Qrt = apply_rope(Qrw, Pos, RopeTable, ?NUM_HEADS,    ?HEAD_DIM), W6 = UsNow(),
            Krt = apply_rope(Krw, Pos, RopeTable, ?NUM_KV_HEADS, ?HEAD_DIM), W7 = UsNow(),
            {Aow, _, _} = attention_gqa(Qrt, Krt, Vrw, [], []), W8 = UsNow(),
            Aof = floats_to_fp16(Aow), W9 = UsNow(),
            Oow = linear_fp8_w8a16(Aof, Ob, ?HIDDEN), W10 = UsNow(),
            H1w = list_add(HiddenIn, Oow), W11 = UsNow(),
            X2w = rmsnorm(H1w, N2b, ?EPS), W12 = UsNow(),
            X2f = floats_to_fp16(X2w), W13 = UsNow(),
            Gow = linear_fp8_w8a16(X2f, Gb, ?FFN), W14 = UsNow(),
            Uow = linear_fp8_w8a16(X2f, Ub, ?FFN), W15 = UsNow(),
            Sw = lists:zipwith(fun(Gv, Uv) -> silu(Gv) * Uv end, Gow, Uow), W16 = UsNow(),
            Swf = floats_to_fp16(Sw), W17 = UsNow(),
            _Fw = linear_fp8_w8a16(Swf, Db, ?HIDDEN), W18 = UsNow(),
            _ = list_add(H1w, _Fw), W19 = UsNow(),
            Total2 = W19 - W0,
            Stages2 = [
                {rmsnorm_1, W1 - W0},
                {floats_to_fp16_x1, W2 - W1},
                {linear_Q, W3 - W2},
                {linear_K, W4 - W3},
                {linear_V, W5 - W4},
                {rope_Q, W6 - W5},
                {rope_K, W7 - W6},
                {attention_gqa, W8 - W7},
                {floats_to_fp16_attn, W9 - W8},
                {linear_O, W10 - W9},
                {residual_1, W11 - W10},
                {rmsnorm_2, W12 - W11},
                {floats_to_fp16_x2, W13 - W12},
                {linear_gate, W14 - W13},
                {linear_up, W15 - W14},
                {silu_mul, W16 - W15},
                {floats_to_fp16_sw, W17 - W16},
                {linear_down, W18 - W17},
                {residual_2, W19 - W18}
            ],
            io:format("Stage (warm)                us    pct~n"),
            [io:format("~-25s ~6w  ~6.2f~n",
                       [Name, Us, 100.0 * Us / float(Total2)])
             || {Name, Us} <- Stages2],
            io:format("~-25s ~6w~n", ["TOTAL (warm layer)", Total2]),
            io:format("Projected per-token (22 warm): ~p ms~n",
                      [Total2 * 22 div 1000]);
        false -> ok
    end,
    io:format("~n--- Cold pass (layer 0, includes init) ---~n"),
