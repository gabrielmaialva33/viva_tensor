 ...
 */
ERL_NIF_TERM ct_from_list(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
 ...
 */
ERL_NIF_TERM ct_to_list(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
...
ERL_NIF_TERM ct_shape(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    CudaTensor *t = get_cuda_tensor(env, argv[0]);
    if (!t)
        return make_error(env, "invalid_cuda_tensor");
 ...
 */
ERL_NIF_TERM ct_matmul(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    ...
    int result = cuda_sgemm_gpu(m, n, k, 1.0f, a->d_data, k, b->d_data, n, 0.0f, c->d_data, n);
 ...
 */
ERL_NIF_TERM ct_matmul_inplace_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    CudaTensor *a = get_cuda_tensor(env, argv[0]);
    CudaTensor *b = get_cuda_tensor(env, argv[1]);
    ...
    int result =
        cuda_sgemm_gpu_inplace(m, n, k, 1.0f, a->d_data, k, b->d_data, n, 0.0f, c->d_data, n);
 ...
 */
ERL_NIF_TERM ct_matmul_async(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    CudaTensor *a = get_cuda_tensor(env, argv[0]);
    CudaTensor *b = get_cuda_tensor(env, argv[1]);
    if (!a || !b)
    ...
    /* SGEMM async - no sync! */
    int result =
        cuda_sgemm_gpu_async(m, n, k, 1.0f, a->d_data, k, b->d_data, n, 0.0f, c->d_data, n);
