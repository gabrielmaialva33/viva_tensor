...
int register_packed_weight_resource(ErlNifEnv *env) {
    PACKED_WEIGHT_RES = enif_open_resource_type(env, NULL, "PackedWeight", packed_weight_destructor,
...
int register_embedding_table_resource(ErlNifEnv *env) {
    EMBEDDING_TABLE_RES =
        enif_open_resource_type(env, NULL, "EmbeddingTable", embedding_table_destructor,
                                ERL_NIF_RT_CREATE, NULL);
    return EMBEDDING_TABLE_RES != NULL ? 0 : -1;
}

PackedWeight *alloc_packed_weight(void) {
    PackedWeight *w = (PackedWeight *)enif_alloc_resource(PACKED_WEIGHT_RES, sizeof(PackedWeight));
    if (!w)
        return NULL;
...
ERL_NIF_TERM nt_embedding_table_new(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    if (!enif_inspect_binary(env, argv[0], &bf16_bin))
        return make_error(env, "invalid_embedding");
    if (!enif_get_int(env, argv[1], &vocab) || vocab <= 0)
        return make_error(env, "invalid_vocab");
    if (!enif_get_int(env, argv[2], &hidden) || hidden <= 0)
        return make_error(env, "invalid_hidden");

    size_t elems = (size_t)vocab * (size_t)hidden;
    if (bf16_bin.size != elems * sizeof(uint16_t))
        return make_error(env, "embedding_size_mismatch");

    uint16_t *host_fp16 = (uint16_t *)enif_alloc(elems * sizeof(uint16_t));
    if (!host_fp16)
        return make_error(env, "alloc_embedding_host_failed");
    ...
    EmbeddingTable *table =
        (EmbeddingTable *)enif_alloc_resource(EMBEDDING_TABLE_RES, sizeof(EmbeddingTable));
    if (!table) {
        enif_free(host_fp16);
    ...
    if (cudaMalloc(&table->d_weight, table->bytes) != cudaSuccess ||
        cudaMemcpy(table->d_weight, host_fp16, table->bytes, cudaMemcpyHostToDevice) !=
            cudaSuccess) {
        enif_free(host_fp16);
...
ERL_NIF_TERM nt_embedding_table_new_fp16(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    int vocab = 0, hidden = 0;
    if (!enif_inspect_binary(env, argv[0], &fp16_bin))
        return make_error(env, "invalid_embedding");
    if (!enif_get_int(env, argv[1], &vocab) || vocab <= 0)
        return make_error(env, "invalid_vocab");
    if (!enif_get_int(env, argv[2], &hidden) || hidden <= 0)
    ...
    size_t elems = (size_t)vocab * (size_t)hidden;
    if (fp16_bin.size != elems * sizeof(uint16_t))
        return make_error(env, "embedding_size_mismatch");
    ...
    if (cudaMalloc(&table->d_weight, table->bytes) != cudaSuccess ||
        cudaMemcpy(table->d_weight, fp16_bin.data, table->bytes, cudaMemcpyHostToDevice) !=
            cudaSuccess) {
        enif_release_resource(table);
        return make_error(env, "cuda_upload_embedding_failed");
    }
