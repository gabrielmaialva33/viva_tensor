...
static int ensure_init() {
    if (g_ctx) return 0;
    if (cublasLtCreate(&g_ctx) != CUBLAS_STATUS_SUCCESS) return -1;
...
int cublaslt_fp16_fused_bench(int M, int N, int K, int iters, int epilogue) {
    ...
    if (cudaMalloc((void**)&d_bias, bytes_bias) != cudaSuccess) {
        cudaFree(d_A); cudaFree(d_BT); cudaFree(d_C); return -15;
    }
    ...
    if (cublasLtMatmulDescCreate(&desc, CUBLAS_COMPUTE_16F, CUDA_R_16F)
        ...
        cudaFree(d_A); cudaFree(d_BT); cudaFree(d_C); cudaFree(d_bias);
        return -16;
    }
    ...
    cublasStatus_t st = cublasLtMatmulAlgoGetHeuristic(
        g_ctx, desc, layout_bt, layout_a, layout_c, layout_c,
    ...
    /* Warmup */
    st = cublasLtMatmul(g_ctx, desc, &alpha_h,
        d_BT, layout_bt, d_A, layout_a,
        &beta_h,
        d_C, layout_c, d_C, layout_c,
        &result.algo, g_workspace, g_workspace_size, (cudaStream_t)0);
    ...
    for (int i = 0; i < iters; ++i) {
        cublasLtMatmul(g_ctx, desc, &alpha_h,
            d_BT, layout_bt, d_A, layout_a,
            &beta_h,
            d_C, layout_c, d_C, layout_c,
            &result.algo, g_workspace, g_workspace_size, (cudaStream_t)0);
