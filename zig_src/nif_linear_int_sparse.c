...
#ifndef _WIN32
extern int cutlass_int8_sparse_run(
    int M, int N, int K,
...
extern int cutlass_int4_sparse_run(
    int M, int N, int K,
    const void*   d_A_packed,
    const void*   d_B,
    int32_t*      d_C,
    const void*   d_E,
    void*         workspace,
...
static void dequant_to_fp16(
    const int32_t* h_acc,      /* [B, out_f] */
    ...
    uint16_t* h_out_fp16)
{
    for (int b = 0; b < B; ++b) {
        ...
        for (int o = 0; o < out_f; ++o) {
            float val = (float)h_acc[(size_t)b * (size_t)out_f + o]
                        * h_scales_o[o] * st;
            if (h_bias_fp16) {
 ...
 * ========================================================================= */
ERL_NIF_TERM nt_linear_int8_sparse(ErlNifEnv* env, int argc,
    ...
    if (B <= 0) return make_error(env, "empty_input");
    ...
    if (cerr != cudaSuccess) goto fail_alloc;
    ...
    if (!d_B_colmajor) goto fail_alloc;
    for (int b = 0; b < B; ++b) {
        for (int k = 0; k < in_f; ++k) {
            d_B_colmajor[(size_t)b * in_f + k] =
                ctx.h_input_q[(size_t)b * in_f + k];
        }
    }
    ...
    cerr = cudaMemcpy(d_B, ctx.h_input_q, (size_t)in_f * (size_t)B,
                      cudaMemcpyHostToDevice);
    free(d_B_colmajor); d_B_colmajor = NULL;
    if (cerr != cudaSuccess) goto fail_alloc;
    ...
    if (w->cusparselt_plan && w->d_compressed && B == 128) {
        ...
        if (cusparseLtInit(&handle) == CUSPARSE_STATUS_SUCCESS) {
            ...
            cusparseStatus_t st = cusparseLtMatmul(
                &handle, plan, &alpha,
                w->d_compressed, d_B, &beta, d_C, d_C,
                workspace, NULL, 0);
    ...
    if (!h_acc) goto fail_alloc;
    cerr = cudaMemcpy(h_acc, d_C,
                      (size_t)out_f * (size_t)B * sizeof(int32_t),
                      cudaMemcpyDeviceToHost);
    if (cerr != cudaSuccess) { free(h_acc); goto fail_alloc; }

    /* Per-channel scales reside on device; copy to host for dequant. */
    float* h_chan_scales = (float*)malloc((size_t)out_f * sizeof(float));
    if (!h_chan_scales) { free(h_acc); goto fail_alloc; }
    cudaMemcpy(h_chan_scales, w->d_scales,
               (size_t)out_f * sizeof(float), cudaMemcpyDeviceToHost);
    ...
    for (int b = 0; b < B; ++b) {
        for (int o = 0; o < out_f; ++o) {
            ...
            if (ctx.h_bias_fp16) v += f16_to_f32(ctx.h_bias_fp16[o]);
 ...
 * ========================================================================= */
ERL_NIF_TERM nt_linear_int4_sparse(ErlNifEnv* env, int argc,
                                   const ERL_NIF_TERM argv[]) {
    (void)argc;
#ifdef _WIN32
    return make_error(env, "cuda_unavailable_on_windows");
    ...
    free(h_packed_input);
    if (cerr != cudaSuccess) goto fail4_alloc;
    ...
    /* Download + dequant. */
    int32_t* h_acc = (int32_t*)malloc((size_t)out_f * (size_t)B * sizeof(int32_t));
    if (!h_acc) goto fail4_alloc;
    cerr = cudaMemcpy(h_acc, d_C,
                      (size_t)out_f * (size_t)B * sizeof(int32_t),
                      cudaMemcpyDeviceToHost);
    if (cerr != cudaSuccess) { free(h_acc); goto fail4_alloc; }

    float* h_chan_scales = (float*)malloc((size_t)out_f * sizeof(float));
    if (!h_chan_scales) { free(h_acc); goto fail4_alloc; }
    cudaMemcpy(h_chan_scales, w->d_scales,
               (size_t)out_f * sizeof(float), cudaMemcpyDeviceToHost);

    ErlNifBinary out_bin;
    if (!enif_alloc_binary((size_t)B * (size_t)out_f * sizeof(uint16_t), &out_bin)) {
        free(h_acc); free(h_chan_scales); goto fail4_alloc;
    }
    uint16_t* h_out = (uint16_t*)out_bin.data;
