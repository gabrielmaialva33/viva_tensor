...
ERL_NIF_TERM nif_sage_available(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    (void)argv;
    return sage_available() ? enif_make_atom(env, "true") : enif_make_atom(env, "false");
}

/** sage_fp8_available() -> true | false */
ERL_NIF_TERM nif_sage_fp8_available(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    (void)argv;
    return sage_fp8_available() ? enif_make_atom(env, "true") : enif_make_atom(env, "false");
}

/** sage_quant_int8(Tensor, BlockSize) -> {ok, {QuantTensor, Scales}}
 *  Per-block INT8 quantization for SageAttention.
 */
ERL_NIF_TERM nif_sage_quant_int8(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 2)
 ...
 */
ERL_NIF_TERM nif_sage_softmax(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 2)
        return enif_make_badarg(env);
    ...
    size_t n = (size_t)t->size;
    if (n % dim != 0)
        return make_error(env, "size_not_divisible_by_dim");
 ...
 */
ERL_NIF_TERM nif_sage_attention(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 8)
        return enif_make_badarg(env);
    ...
    if (!enif_get_int(env, argv[3], &batch) || !enif_get_int(env, argv[4], &heads) ||
        !enif_get_int(env, argv[5], &seq_q) || !enif_get_int(env, argv[6], &seq_k) ||
    ...
    if ((size_t)q->size != q_expected)
        return make_error(env, "q_size_mismatch");
    if ((size_t)k->size != k_expected)
        return make_error(env, "k_size_mismatch");
    if ((size_t)v->size != v_expected)
        return make_error(env, "v_size_mismatch");
    ...
    for (size_t i = 0; i < q_expected; i++)
        q_fp32[i] = (float)q->data[i];
    for (size_t i = 0; i < k_expected; i++)
        k_fp32[i] = (float)k->data[i];
    for (size_t i = 0; i < v_expected; i++)
        v_fp32[i] = (float)v->data[i];

    float sm_scale = 1.0f / sqrtf((float)head_dim);

    int result = sage_attention_cpu(o_fp32, q_fp32, k_fp32, v_fp32, batch, heads, seq_q, seq_k,
                                    head_dim, sm_scale);
 ...
 */
ERL_NIF_TERM sage_attention_ct(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 8)
        return enif_make_badarg(env);
    ...
    int result = sage_attention_gpu(out->d_data, q->d_data, k->d_data, v->d_data, batch, heads,
                                    seq_q, seq_k, head_dim, sm_scale);
