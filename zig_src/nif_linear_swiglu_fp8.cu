...
static int swiglu_quantize_fp8_e4m3_host(uint8_t *d_out, const float *h_in, size_t n,
                                         float *out_scale) {
    float amax = 0.0f;
...
__global__ void silu_mul_kernel(const float *__restrict__ gate, const float *__restrict__ up,
                                ...
                                const float *__restrict__ up_scales, float input_scale,
                                float *__restrict__ out, int total, int out_features) {
...
static int launch_silu_mul(const float *d_gate, const float *d_up, const __half *d_bias,
                           const float *d_gate_scales, const float *d_up_scales, float input_scale,
                           float *d_out, int batch, int out_features) {
    int total = batch * out_features;
    int threads = 256;
    int blocks = (total + threads - 1) / threads;
    silu_mul_kernel<<<blocks, threads>>>(d_gate, d_up, d_bias, d_gate_scales, d_up_scales,
                                         input_scale, d_out, total, out_features);
...
extern "C" ERL_NIF_TERM nt_linear_swiglu_fp8_nif(ErlNifEnv *env, int argc,
    ...
    if (!h_input_fp32) {
        free(input_doubles);
        return make_error(env, "oom_host");
    }
    for (unsigned i = 0; i < data_len; ++i)
        h_input_fp32[i] = (float)input_doubles[i];
    free(input_doubles);
    ...
    if (!gate_w->d_scales || !up_w->d_scales || gate_w->scales_count != (size_t)out_features ||
    ...
    int rc1 = cutlass_fp8_gemm_f32acc_out_f32(batch, out_features, in_features, d_input_fp8,
                                              d_gate_weight, d_gate_out);
    if (rc1 != 0) {
        cudaFree(d_out);
        cudaFree(d_up_out);
        cudaFree(d_gate_out);
    ...
    int rc2 = cutlass_fp8_gemm_f32acc_out_f32(batch, out_features, in_features, d_input_fp8,
                                              d_up_weight, d_up_out);
    if (rc2 != 0) {
        cudaFree(d_out);
        cudaFree(d_up_out);
        cudaFree(d_gate_out);
    ...
    int kc =
        launch_silu_mul(d_gate_out, d_up_out, d_bias, (const float *)gate_w->d_scales,
                        (const float *)up_w->d_scales, input_scale, d_out, batch, out_features);
    cudaFree(d_gate_out);
    cudaFree(d_up_out);
    if (d_bias)
        cudaFree(d_bias);
