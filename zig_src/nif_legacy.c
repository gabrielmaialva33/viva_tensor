...
ERL_NIF_TERM nif_simd_dot(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 2)
...
ERL_NIF_TERM nif_simd_sum(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 1)
        return enif_make_badarg(env);
    unsigned len;
...
ERL_NIF_TERM nif_simd_scale(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 2)
        return enif_make_badarg(env);
    unsigned len;
    double *d = list_to_doubles(env, argv[0], &len);
...
ERL_NIF_TERM nif_simd_add(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 2)
        return enif_make_badarg(env);
    unsigned la, lb;
    double *a = list_to_doubles(env, argv[0], &la);
...
ERL_NIF_TERM nif_simd_mul(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 2)
        return enif_make_badarg(env);
    unsigned la, lb;
    double *a = list_to_doubles(env, argv[0], &la);
...
ERL_NIF_TERM nif_simd_matmul(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 5)
        return enif_make_badarg(env);
    int mi, ni, ki;
    ...
    cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, mi, ni, ki, 1.0, a, ki, b, ni, 0.0, c,
                ni);
#else
    if (g_dgemm) {
...
ERL_NIF_TERM nif_simd_available(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    (void)argv;
    return enif_make_atom(env, "true");
}

ERL_NIF_TERM nif_backend_info(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    (void)argv;
    char info[512];
#if defined(_WIN32) || defined(USE_MKL_DIRECT)
    ...
    snprintf(
        ...
        blas_name, g_cpu_info.physical_cores, g_cpu_info.logical_cpus, g_cpu_info.l2_cache_kb,
        g_cpu_info.l3_cache_kb,
        g_cpu_info.has_avx512 ? "AVX-512 " : (g_cpu_info.has_avx2 ? "AVX2 " : ""),
        g_cpu_info.has_hybrid ? "Hybrid " : "", g_cpu_info.optimal_threads);
    ErlNifBinary bin;
...
/** cpu_topology() -> {ok, Map} */
ERL_NIF_TERM nif_cpu_topology(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    (void)argv;
