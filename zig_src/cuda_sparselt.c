...
typedef struct {
    __attribute__((aligned(16))) uint8_t data[512];
} cusparseLtHandle_t;
typedef struct {
    __attribute__((aligned(16))) uint8_t data[512];
...
typedef enum { CUSPARSE_ORDER_COL = 0, CUSPARSE_ORDER_ROW = 1 } cusparseOrder_t;
...
typedef enum { CUSPARSELT_SPARSITY_50_PERCENT = 0 } cusparseLtSparsity_t;

typedef enum { CUSPARSELT_MATMUL_ALG_DEFAULT = 0 } cusparseLtMatmulAlg_t;
...
typedef cusparseStatus_t (*cusparseLtStructuredDescriptorInit_fn)(
    const cusparseLtHandle_t *handle, cusparseLtMatDescriptor_t *matDescr, int64_t rows,
    int64_t cols, int64_t ld, uint32_t alignment, int valueType, /* cudaDataType as int */
...
typedef cusparseStatus_t (*cusparseLtMatmulDescriptorInit_fn)(
    ...
    cusparseOperation_t opA, cusparseOperation_t opB, const cusparseLtMatDescriptor_t *matA,
    const cusparseLtMatDescriptor_t *matB, const cusparseLtMatDescriptor_t *matC,
...
typedef cusparseStatus_t (*cusparseLtMatmulPlanDestroy_fn)(const cusparseLtMatmulPlan_t *plan);

typedef cusparseStatus_t (*cusparseLtSpMMAPrune_fn)(const cusparseLtHandle_t *handle,
                                                    const cusparseLtMatmulDescriptor_t *matmulDescr,
...
typedef cusparseStatus_t (*cusparseLtSpMMAPrune2_fn)(
    const cusparseLtHandle_t *handle, const cusparseLtMatDescriptor_t *sparseMatDescr,
...
typedef cusparseStatus_t (*cusparseLtSpMMACompressedSize_fn)(const cusparseLtHandle_t *handle,
...
typedef cusparseStatus_t (*cusparseLtSpMMACompress_fn)(const cusparseLtHandle_t *handle,
                                                       const cusparseLtMatmulPlan_t *plan,
                                                       const void *d_dense, void *d_compressed,
                                                       void *d_compressBuffer, cudaStream_t stream);

typedef cusparseStatus_t (*cusparseLtSpMMACompress2_fn)(
    const cusparseLtHandle_t *handle, const cusparseLtMatDescriptor_t *sparseMatDescr,
    int isSparseA, cusparseOperation_t op, const void *d_dense, void *d_compressed,
    void *d_compressedBuffer, cudaStream_t stream);

typedef cusparseStatus_t (*cusparseLtMatmulGetWorkspace_fn)(const cusparseLtHandle_t *handle,
                                                            const cusparseLtMatmulPlan_t *plan,
                                                            size_t *workspaceSize);

typedef cusparseStatus_t (*cusparseLtMatmul_fn)(const cusparseLtHandle_t *handle,
                                                const cusparseLtMatmulPlan_t *plan,
...
typedef cusparseStatus_t (*cusparseLtMatmulSearch_fn)(const cusparseLtHandle_t *handle,
...
typedef cusparseStatus_t (*cusparseLtMatmulDescSetAttribute_fn)(
    const cusparseLtHandle_t *handle, cusparseLtMatmulDescriptor_t *matmulDescr, int attribute,
    const void *data, size_t dataSize);
...
int cusparselt_init(void) {
    ...
    const char *paths[] = {
        "libcusparseLt.so", "libcusparseLt.so.0", "/usr/local/cuda/lib64/libcusparseLt.so",
    ...
    if (!g_cusparselt_lib) {
        ...
        const char *pip_paths[] = {"/home/gabriel-maia/.asdf/installs/python/3.14.3/lib/python3.14/"
                                   "site-packages/nvidia/cusparselt/lib/libcusparseLt.so.0",
                                   NULL};
    ...
    if (!g_cusparselt_lib) {
        fprintf(stderr, "[viva_tensor] cuSPARSELt: library not found (install: pip install "
                        "nvidia-cusparselt)\n");
        g_cusparselt_available = 0;
        return 0;
    }
...
#define LOAD_FN(name)                                                            \
    ...
    g_cusparseLtSpMMACompressedSize2 =
        (cusparseLtSpMMACompressedSize2_fn)dlsym(g_cusparselt_lib,
                                                 "cusparseLtSpMMACompressedSize2");
    g_cusparseLtSpMMACompress2 =
        (cusparseLtSpMMACompress2_fn)dlsym(g_cusparselt_lib, "cusparseLtSpMMACompress2");
    g_cusparseLtSpMMAPrune2 =
        (cusparseLtSpMMAPrune2_fn)dlsym(g_cusparselt_lib, "cusparseLtSpMMAPrune2");
    g_cusparseLtMatmulAlgSelectionDestroy =
        (cusparseLtMatmulAlgSelectionDestroy_fn)dlsym(g_cusparselt_lib,
                                                      "cusparseLtMatmulAlgSelectionDestroy");
    g_cusparseLtGetErrorString =
        (cusparseLtGetErrorString_fn)dlsym(g_cusparselt_lib, "cusparseLtGetErrorString");
    g_cusparseLtGetErrorName =
        (cusparseLtGetErrorName_fn)dlsym(g_cusparselt_lib, "cusparseLtGetErrorName");
    g_cusparseLtMatmulAlgSetAttribute =
        (cusparseLtMatmulAlgSetAttribute_fn)dlsym(g_cusparselt_lib,
                                                  "cusparseLtMatmulAlgSetAttribute");
    g_cusparseLtMatmulAlgGetAttribute =
        (cusparseLtMatmulAlgGetAttribute_fn)dlsym(g_cusparselt_lib,
                                                  "cusparseLtMatmulAlgGetAttribute");
    g_cusparseLtMatmulDescSetAttribute =
        (cusparseLtMatmulDescSetAttribute_fn)dlsym(g_cusparselt_lib,
...
#undef LOAD_FN
     ...
     * Non-blocking avoids implicit sync with default stream = less pipeline stalls. */
    {
        ...
        if (!cudart)
            cudart = dlopen("libcudart.so.12", RTLD_LAZY);
        if (cudart) {
            g_cuda_stream_create =
                (cuda_stream_create_fn)dlsym(cudart, "cudaStreamCreateWithFlags");
            ...
            g_cuda_stream_begin_capture =
                (cuda_stream_begin_capture_fn)dlsym(cudart, "cudaStreamBeginCapture");
            g_cuda_stream_end_capture =
                (cuda_stream_end_capture_fn)dlsym(cudart, "cudaStreamEndCapture");
            g_cuda_graph_instantiate =
                (cuda_graph_instantiate_fn)dlsym(cudart, "cudaGraphInstantiateWithFlags");
            g_cuda_graph_launch = (cuda_graph_launch_fn)dlsym(cudart, "cudaGraphLaunch");
            g_cuda_graph_destroy = (cuda_graph_destroy_fn)dlsym(cudart, "cudaGraphDestroy");
            g_cuda_graph_exec_destroy =
                (cuda_graph_exec_destroy_fn)dlsym(cudart, "cudaGraphExecDestroy");
            /* Don't close cudart — already loaded globally */
        }
    }

    fprintf(stderr,
            "[viva_tensor] cuSPARSELt v0.8 loaded — 2:4 sparsity ready "
            "(%d MiB workspace, stream=%s)\n",
            SPARSELT_WORKSPACE_SIZE / (1024 * 1024), g_sparse_stream ? "dedicated" : "default");
...
int sparse_tensor_create_fp16(const uint16_t *d_dense, int64_t rows, int64_t cols,
                              SparseTensorInternal *out_sparse) {
    if (!cusparselt_available())
        return -1;
    if (!d_dense || !out_sparse)
        return -2;

    /* Dimensions must be multiples of 16 for FP16 2:4 sparsity */
    if (rows % 16 != 0 || cols % 16 != 0) {
        fprintf(stderr,
                "[viva_tensor] SparseTensor: FP16 requires dims multiples of 16 (got %ldx%ld)\n",
                rows, cols);
        return -3;
    }
    ...
    status =
        g_cusparseLtStructuredDescriptorInit(&g_cusparselt_handle, matA, rows, cols, ld,
    ...
    if (g_cusparseLtSpMMAPrune2) {
        status = g_cusparseLtSpMMAPrune2(&g_cusparselt_handle, matA, 1, /* isSparseA = true */
                                         CUSPARSE_OPERATION_NON_TRANSPOSE, d_pruned, d_pruned,
        ...
        g_cusparseLtDenseDescriptorInit(&g_cusparselt_handle, &tmpB, cols, cols, cols, 16,
                                        SPLT_CUDA_R_16F, CUSPARSE_ORDER_ROW);
        g_cusparseLtDenseDescriptorInit(&g_cusparselt_handle, &tmpC, rows, cols, cols, 16,
                                        SPLT_CUDA_R_16F, CUSPARSE_ORDER_ROW);
        g_cusparseLtMatmulDescriptorInit(&g_cusparselt_handle, &tmpMatmul,
                                         CUSPARSE_OPERATION_NON_TRANSPOSE,
                                         CUSPARSE_OPERATION_NON_TRANSPOSE, matA, &tmpB, &tmpC,
                                         &tmpC, CUSPARSE_COMPUTE_16F);

        status = g_cusparseLtSpMMAPrune(&g_cusparselt_handle, &tmpMatmul, d_pruned, d_pruned,
                                        CUSPARSELT_PRUNE_SPMMA_STRIP, NULL);
    ...
    if (g_cusparseLtSpMMACompressedSize2) {
        status =
            g_cusparseLtSpMMACompressedSize2(&g_cusparselt_handle, matA,
        ...
        g_cusparseLtMatmulDescriptorInit(&g_cusparselt_handle, &tmpMatmul,
                                         ...
                                         CUSPARSE_OPERATION_NON_TRANSPOSE, matA, &tmpB, &tmpC,
                                         &tmpC, CUSPARSE_COMPUTE_16F);
        g_cusparseLtMatmulAlgSelectionInit(&g_cusparselt_handle, &tmpAlg, &tmpMatmul,
                                           CUSPARSELT_MATMUL_ALG_DEFAULT);
        g_cusparseLtMatmulPlanInit(&g_cusparselt_handle, &tmpPlan, &tmpMatmul, &tmpAlg);

        status =
            g_cusparseLtSpMMACompressedSize(&g_cusparselt_handle, &tmpPlan,
                                            &out_sparse->compressed_size, &compress_buffer_size);
    ...
    if (g_cusparseLtSpMMACompress2) {
        status = g_cusparseLtSpMMACompress2(&g_cusparselt_handle, matA, 1, /* isSparseA = true */
                                            CUSPARSE_OPERATION_NON_TRANSPOSE, d_pruned,
                                            out_sparse->d_compressed, d_compress_buffer, NULL);
    } else {
        ...
        status = g_cusparseLtSpMMACompress(&g_cusparselt_handle, &tmpPlan, d_pruned,
                                           out_sparse->d_compressed, d_compress_buffer, NULL);
 ...
 */
static int sparse_get_or_create_plan(SparseMatmulPlanCache *cache, SparseTensorInternal *sparse,
                                     int64_t N, const void *d_A_compressed, const void *d_B,
                                     void *d_C, cusparseOperation_t opB) {
    ...
    status = g_cusparseLtStructuredDescriptorInit(&g_cusparselt_handle, &cache->matA_desc, M, K, K,
                                                  16, SPLT_CUDA_R_16F, CUSPARSE_ORDER_ROW,
    ...
    status = g_cusparseLtDenseDescriptorInit(&g_cusparselt_handle, &cache->matB_desc, B_rows,
                                             B_cols, B_ld, 16, SPLT_CUDA_R_16F, CUSPARSE_ORDER_ROW);
    if (status != CUSPARSE_STATUS_SUCCESS) {
    ...
    status = g_cusparseLtDenseDescriptorInit(&g_cusparselt_handle, &cache->matC_desc, M, N, N, 16,
                                             SPLT_CUDA_R_16F, CUSPARSE_ORDER_ROW);
    if (status != CUSPARSE_STATUS_SUCCESS) {
    ...
    status =
        g_cusparseLtMatmulDescriptorInit(&g_cusparselt_handle, &cache->matmul_desc,
                                         CUSPARSE_OPERATION_NON_TRANSPOSE, opB, &cache->matA_desc,
                                         &cache->matB_desc, &cache->matC_desc, &cache->matC_desc,
    ...
    if (g_cusparseLtMatmulDescSetAttribute && d_A_compressed) {
        g_cusparseLtMatmulDescSetAttribute(&g_cusparselt_handle, &cache->matmul_desc,
                                           CUSPARSELT_MATMUL_SPARSE_MAT_POINTER, &d_A_compressed,
                                           sizeof(d_A_compressed));
    }
    ...
    if (g_cusparseLtMatmulAlgSetAttribute) {
        ...
        g_cusparseLtMatmulAlgSetAttribute(&g_cusparselt_handle, &cache->alg_sel,
                                          CUSPARSELT_MATMUL_SEARCH_ITERATIONS, &search_iters,
                                          sizeof(search_iters));
    }

    /* Plan init */
    status = g_cusparseLtMatmulPlanInit(&g_cusparselt_handle, &cache->plan, &cache->matmul_desc,
                                        &cache->alg_sel);
    if (status != CUSPARSE_STATUS_SUCCESS) {
    ...
    if (g_cusparseLtMatmulGetWorkspace) {
        ...
        if (status == CUSPARSE_STATUS_SUCCESS && needed > SPARSELT_WORKSPACE_SIZE) {
            ...
            if (g_cuda_malloc(&big_ws, needed) == 0) {
                ...
                fprintf(stderr,
                        "[viva_tensor] sparse_plan: allocated %zu MiB workspace (split-K)\n",
                        needed / (1024 * 1024));
            }
        }
    }
    ...
    if (d_A_compressed && d_B && d_C) {
        ...
        status = g_cusparseLtMatmulSearch(&g_cusparselt_handle, &cache->plan, &alpha_f,
                                          d_A_compressed, d_B, &beta_f, d_C, d_C,
                                          cache->d_workspace, stream_ptr, num_streams);
        if (status != CUSPARSE_STATUS_SUCCESS) {
            fprintf(stderr,
                    "[viva_tensor] sparse_plan: MatmulSearch failed: %d (using default alg)\n",
                    status);
        }
    }
    ...
    if (g_cusparseLtMatmulAlgGetAttribute) {
        ...
        g_cusparseLtMatmulAlgGetAttribute(&g_cusparselt_handle, &cache->alg_sel,
                                          CUSPARSELT_MATMUL_ALG_CONFIG_MAX_ID, &max_alg_id,
                                          sizeof(max_alg_id));
        g_cusparseLtMatmulAlgGetAttribute(&g_cusparselt_handle, &cache->alg_sel,
                                          CUSPARSELT_MATMUL_SPLIT_K, &split_k, sizeof(split_k));
        g_cusparseLtMatmulAlgGetAttribute(&g_cusparselt_handle, &cache->alg_sel,
                                          CUSPARSELT_MATMUL_SPLIT_K_MODE, &split_k_mode,
                                          sizeof(split_k_mode));
        g_cusparseLtMatmulAlgGetAttribute(&g_cusparselt_handle, &cache->alg_sel,
                                          CUSPARSELT_MATMUL_SPLIT_K_BUFFERS, &split_k_buffers,
                                          sizeof(split_k_buffers));
        fprintf(
            stderr,
            "[viva_tensor] sparse_plan: ready %ldx%ldx%ld (alg %d/%d, splitK=%d mode=%s bufs=%d)\n",
            M, N, K, alg_id, max_alg_id, split_k,
            split_k_mode == 0   ? "1kern"
            : split_k_mode == 1 ? "2kern"
                                : "?",
            split_k_buffers);
    } else {
 ...
 */
int sparse_matmul_fp16(SparseTensorInternal *sparse, const uint16_t *d_B, uint16_t *d_C, int64_t N,
    ...
    int rc = sparse_get_or_create_plan(&g_sparse_fp16_cache, sparse, N, sparse->d_compressed, d_B,
                                       d_C, CUSPARSE_OPERATION_NON_TRANSPOSE);
    if (rc != 0)
        return rc;
    ...
    const void *alpha_ptr =
        g_sparse_fp16_cache.compute_is_32f ? (const void *)&alpha_f : (const void *)&alpha_h;
    const void *beta_ptr =
        g_sparse_fp16_cache.compute_is_32f ? (const void *)&beta_f : (const void *)&beta_h;

    cudaStream_t *stream_ptr = g_sparse_stream ? &g_sparse_stream : NULL;
    int num_streams = g_sparse_stream ? 1 : 0;

    cusparseStatus_t status =
        g_cusparseLtMatmul(&g_cusparselt_handle, &g_sparse_fp16_cache.plan, alpha_ptr,
                           sparse->d_compressed, d_B, beta_ptr, d_C, d_C,
                           g_sparse_fp16_cache.d_workspace, stream_ptr, num_streams);
 ...
 */
int sparse_matmul_fp16_bench(SparseTensorInternal *sparse, const uint16_t *d_B, uint16_t *d_C,
                             int64_t N, int iters) {
    if (!cusparselt_available())
        return -1;
    if (!sparse || !d_B || !d_C)
    ...
    if (g_sparse_stream && g_cuda_stream_begin_capture && g_cuda_stream_end_capture &&
        g_cuda_graph_instantiate && g_cuda_graph_launch && g_cuda_graph_destroy &&
        g_cuda_graph_exec_destroy) {
        ...
        if (err == 0) {
            cusparseStatus_t status =
                g_cusparseLtMatmul(&g_cusparselt_handle, &g_sparse_fp16_cache.plan, alpha_ptr,
                                   sparse->d_compressed, d_B, beta_ptr, d_C, d_C,
                                   g_sparse_fp16_cache.d_workspace, &g_sparse_stream, 1);
 ...
 */
int sparse_matmul_fp16_bench_tn(SparseTensorInternal *sparse, const uint16_t *d_B, uint16_t *d_C,
                                int64_t N, int iters) {
    if (!cusparselt_available())
        return -1;
    if (!sparse || !d_B || !d_C)
    ...
    int rc = sparse_get_or_create_plan(&g_sparse_fp16_tn_cache, sparse, N, sparse->d_compressed,
                                       d_B, d_C, CUSPARSE_OPERATION_TRANSPOSE);
    if (rc != 0)
        return rc;
    ...
    for (int i = 0; i < iters; i++) {
        cusparseStatus_t status =
            g_cusparseLtMatmul(&g_cusparselt_handle, &g_sparse_fp16_tn_cache.plan, alpha_ptr,
                               sparse->d_compressed, d_B, beta_ptr, d_C, d_C,
                               g_sparse_fp16_tn_cache.d_workspace, stream_ptr, num_streams);
        if (status != CUSPARSE_STATUS_SUCCESS)
            return -8;
    }
...
static int sparse_int8_check_supported(void) {
    ...
    cusparseStatus_t s;
    s = g_cusparseLtStructuredDescriptorInit(&g_cusparselt_handle, &tmpA, N, N, N, 16,
                                             SPLT_CUDA_R_8I, CUSPARSE_ORDER_ROW,
                                             CUSPARSELT_SPARSITY_50_PERCENT);
    ...
    s = g_cusparseLtDenseDescriptorInit(&g_cusparselt_handle, &tmpB, N, N, N, 16, SPLT_CUDA_R_8I,
                                        CUSPARSE_ORDER_ROW);
    if (s != CUSPARSE_STATUS_SUCCESS) {
        g_sparse_int8_supported = 0;
        goto done_check;
    ...
    s = g_cusparseLtDenseDescriptorInit(&g_cusparselt_handle, &tmpC, N, N, N, 16, SPLT_CUDA_R_8I,
                                        CUSPARSE_ORDER_ROW);
    if (s != CUSPARSE_STATUS_SUCCESS) {
        g_sparse_int8_supported = 0;
        goto done_check;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        s = g_cusparseLtMatmulDescriptorInit(&g_cusparselt_handle, &tmpMatmul,
                                             CUSPARSE_OPERATION_NON_TRANSPOSE,
                                             CUSPARSE_OPERATION_NON_TRANSPOSE, &tmpA, &tmpB, &tmpC,
                                             &tmpC, compute_types[i]);
        if (s == CUSPARSE_STATUS_SUCCESS) {
...
done_check:
    ...
    if (have_B)
        g_cusparseLtMatDescriptorDestroy(&tmpB);
    if (have_A)
        g_cusparseLtMatDescriptorDestroy(&tmpA);
...
int sparse_tensor_create_int8(const int8_t *d_dense, int64_t rows, int64_t cols,
                              SparseTensorInternal *out_sparse) {
    if (!cusparselt_available())
        return -1;
    if (!d_dense || !out_sparse)
    ...
    if (!sparse_int8_check_supported())
        return -100; /* Not supported */

    if (rows % 16 != 0 || cols % 16 != 0) {
        fprintf(stderr,
                "[viva_tensor] SparseTensor INT8: requires dims multiples of 16 (got %ldx%ld)\n",
                rows, cols);
        return -3;
    }
    ...
    status = g_cusparseLtStructuredDescriptorInit(&g_cusparselt_handle, matA, rows, cols, ld, 16,
                                                  SPLT_CUDA_R_8I, CUSPARSE_ORDER_ROW,
                                                  CUSPARSELT_SPARSITY_50_PERCENT);
    if (status != CUSPARSE_STATUS_SUCCESS)
        return -4;
    ...
    if (g_cusparseLtSpMMAPrune2) {
        status =
            g_cusparseLtSpMMAPrune2(&g_cusparselt_handle, matA, 1, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                    d_pruned, d_pruned, CUSPARSELT_PRUNE_SPMMA_STRIP, NULL);
    } else {
        g_cuda_free(d_pruned);
        g_cusparseLtMatDescriptorDestroy(matA);
    ...
    if (g_cusparseLtSpMMACompress2) {
        if (g_cuda_sync)
            g_cuda_sync();
        status = g_cusparseLtSpMMACompress2(&g_cusparselt_handle, matA, 1,
                                            CUSPARSE_OPERATION_NON_TRANSPOSE, d_pruned,
                                            out_sparse->d_compressed, d_compress_buffer, NULL);
    } else {
    ...
    g_cuda_sync();
    fprintf(stderr,
            "[viva_tensor] SparseTensor INT8 created: %ldx%ld -> %zu bytes (%.1f%% of dense)\n",
            rows, cols, out_sparse->compressed_size,
            100.0 * out_sparse->compressed_size / dense_size);
    return 0;
}

static int sparse_get_or_create_plan_int8(SparseMatmulPlanCache *cache,
                                          SparseTensorInternal *sparse, int64_t N,
    ...
    status = g_cusparseLtStructuredDescriptorInit(&g_cusparselt_handle, &cache->matA_desc, M, K, K,
                                                  16, SPLT_CUDA_R_8I, CUSPARSE_ORDER_ROW,
                                                  CUSPARSELT_SPARSITY_50_PERCENT);
    if (status != 0)
        return -1;

    status = g_cusparseLtDenseDescriptorInit(&g_cusparselt_handle, &cache->matB_desc, K, N, N, 16,
                                             SPLT_CUDA_R_8I, CUSPARSE_ORDER_ROW);
    if (status != 0) {
        g_cusparseLtMatDescriptorDestroy(&cache->matA_desc);
        return -2;
    ...
    status = g_cusparseLtMatmulDescriptorInit(&g_cusparselt_handle, &cache->matmul_desc,
                                              CUSPARSE_OPERATION_NON_TRANSPOSE,
                                              CUSPARSE_OPERATION_NON_TRANSPOSE, &cache->matA_desc,
                                              &cache->matB_desc, &cache->matC_desc,
                                              &cache->matC_desc, CUSPARSE_COMPUTE_32I);
    if (status != 0) {
        g_cusparseLtMatDescriptorDestroy(&cache->matA_desc);
        g_cusparseLtMatDescriptorDestroy(&cache->matB_desc);
        g_cusparseLtMatDescriptorDestroy(&cache->matC_desc);
    ...
    if (d_A_compressed && d_B && d_C) {
        float alpha_f = 1.0f, beta_f = 0.0f;
        status =
            g_cusparseLtMatmulSearch(&g_cusparselt_handle, &cache->plan, &alpha_f, d_A_compressed,
                                     d_B, &beta_f, d_C, d_C, g_sparselt_workspace, NULL, 0);
        if (status != 0)
...
int sparse_matmul_int8(SparseTensorInternal *sparse, const int8_t *d_B, int8_t *d_C, int64_t N,
                       float alpha, float beta) {
    if (!cusparselt_available())
        return -1;
    if (!sparse || !d_B || !d_C)
    ...
    int rc = sparse_get_or_create_plan_int8(&g_sparse_int8_cache, sparse, N, sparse->d_compressed,
                                            d_B, d_C);
    if (rc != 0)
        return rc;

    float alpha_f = alpha, beta_f = beta;
    cusparseStatus_t status = g_cusparseLtMatmul(&g_cusparselt_handle, &g_sparse_int8_cache.plan,
                                                 &alpha_f, sparse->d_compressed, d_B, &beta_f, d_C,
                                                 d_C, g_sparselt_workspace, NULL, 0);
    return (status == 0) ? 0 : -8;
}

int sparse_matmul_int8_bench(SparseTensorInternal *sparse, const int8_t *d_B, int8_t *d_C,
                             int64_t N, int iters) {
    if (!cusparselt_available())
        return -1;
    if (!sparse || !d_B || !d_C)
    ...
    for (int i = 0; i < iters; i++) {
        cusparseStatus_t status =
            g_cusparseLtMatmul(&g_cusparselt_handle, &g_sparse_int8_cache.plan, &alpha_f,
                               sparse->d_compressed, d_B, &beta_f, d_C, d_C, g_sparselt_workspace,
                               NULL, 0);
        if (status != 0)
            return -8;
    }
