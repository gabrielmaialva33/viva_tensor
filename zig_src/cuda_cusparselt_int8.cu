...
int cusparselt_int8_sparse_bench(int M, int N, int K, int iters, int mode) {
    ...
    CHECK_CUSPARSE(cusparseLtStructuredDescriptorInit(&handle, &matA, M, K, K, 16, CUDA_R_8I,
                                                      CUSPARSE_ORDER_ROW,
                                                      CUSPARSELT_SPARSITY_50_PERCENT));

    /* B (dense): INT8, COL-major → NN layout */
    CHECK_CUSPARSE(
        cusparseLtDenseDescriptorInit(&handle, &matB, K, N, K, 16, CUDA_R_8I, CUSPARSE_ORDER_COL));

    /* C/D: INT32, ROW-major */
    CHECK_CUSPARSE(
        cusparseLtDenseDescriptorInit(&handle, &matC, M, N, N, 16, CUDA_R_32I, CUSPARSE_ORDER_ROW));

    CHECK_CUSPARSE(
        cusparseLtDenseDescriptorInit(&handle, &matD, M, N, N, 16, CUDA_R_32I, CUSPARSE_ORDER_ROW));

    CHECK_CUSPARSE(cusparseLtMatmulDescriptorInit(&handle, &matmulDescr,
                                                  CUSPARSE_OPERATION_NON_TRANSPOSE,
                                                  CUSPARSE_OPERATION_NON_TRANSPOSE, &matA, &matB,
                                                  &matC, &matD, CUSPARSE_COMPUTE_32I));

    CHECK_CUSPARSE(cusparseLtMatmulAlgSelectionInit(&handle, &algSel, &matmulDescr,
                                                    CUSPARSELT_MATMUL_ALG_DEFAULT));

    /* Increase search iterations for more stable tuning */
    int searchIters = 20;
    cusparseLtMatmulAlgSetAttribute(&handle, &algSel, CUSPARSELT_MATMUL_SEARCH_ITERATIONS,
                                    &searchIters, sizeof(searchIters));

    /* Report algorithm candidate pool size */
    int maxAlgId = 0;
    cusparseLtMatmulAlgGetAttribute(&handle, &algSel, CUSPARSELT_MATMUL_ALG_CONFIG_MAX_ID,
                                    &maxAlgId, sizeof(maxAlgId));
    ...
    if (mode == 2) {
        ...
        cusparseLtMatmulAlgSetAttribute(&handle, &algSel, CUSPARSELT_MATMUL_SPLIT_K_MODE, &sk,
                                        sizeof(sk));
        int skBufs = 8;
        cusparseLtMatmulAlgSetAttribute(&handle, &algSel, CUSPARSELT_MATMUL_SPLIT_K_BUFFERS,
                                        &skBufs, sizeof(skBufs));
    } else if (mode == 3) {
    ...
    cusparseLtMatmulDescSetAttribute(&handle, &matmulDescr, CUSPARSELT_MATMUL_SPARSE_MAT_POINTER,
                                     &d_compressed, sizeof(d_compressed));

    float alpha = 1.0f, beta = 0.0f;

    /* Auto-tune: search for best algorithm */
    CHECK_CUSPARSE(cusparseLtMatmulSearch(&handle, &plan, &alpha, d_compressed, d_B, &beta, d_C,
                                          d_C, d_workspace, nullptr, 0));
    CHECK_CUDA(cudaDeviceSynchronize());

    /* Warmup (10 iterations) */
    for (int w = 0; w < 10; w++) {
        cusparseLtMatmul(&handle, &plan, &alpha, d_compressed, d_B, &beta, d_C, d_C, d_workspace,
                         nullptr, 0);
    }
    CHECK_CUDA(cudaDeviceSynchronize());
...
int cusparselt_fp8_sparse_bench(int M, int N, int K, int iters) {
    ...
    cusparseStatus_t st =
        cusparseLtStructuredDescriptorInit(&handle, &matA, M, K, K, 16, CUDA_R_8F_E4M3,
    ...
    CHECK_CUSPARSE(cusparseLtDenseDescriptorInit(&handle, &matB, K, N, K, 16, CUDA_R_8F_E4M3,
                                                 CUSPARSE_ORDER_COL));
    ...
    CHECK_CUSPARSE(
        cusparseLtDenseDescriptorInit(&handle, &matD, M, N, N, 16, CUDA_R_16F, CUSPARSE_ORDER_ROW));

    CHECK_CUSPARSE(cusparseLtMatmulDescriptorInit(&handle, &matmulDescr,
                                                  CUSPARSE_OPERATION_NON_TRANSPOSE,
                                                  CUSPARSE_OPERATION_NON_TRANSPOSE, &matA, &matB,
                                                  &matC, &matD, CUSPARSE_COMPUTE_32F));
...
int cusparselt_fp16_sparse_bench(int M, int N, int K, int iters) {
    ...
    CHECK_CUSPARSE(cusparseLtStructuredDescriptorInit(&handle, &matA, M, K, K, 16, CUDA_R_16F,
                                                      CUSPARSE_ORDER_ROW,
                                                      CUSPARSELT_SPARSITY_50_PERCENT));

    /* B (dense): FP16, COL-major */
    CHECK_CUSPARSE(
        cusparseLtDenseDescriptorInit(&handle, &matB, K, N, K, 16, CUDA_R_16F, CUSPARSE_ORDER_COL));
