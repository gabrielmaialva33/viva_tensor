...
void cuda_tensor16_destructor(ErlNifEnv *env, void *obj) {
    ...
    if (t->d_data)
        cuda_tensor_free(t->d_data);
    if (t->d_data_t)
        cuda_tensor_free(t->d_data_t);
    if (t->d_acc)
        cuda_tensor_free(t->d_acc);
    if (t->shape)
        free(t->shape);
}
...
CudaTensor16 *alloc_cuda_tensor16(int ndim, const int *shape) {
    CudaTensor16 *t =
        (CudaTensor16 *)enif_alloc_resource(CUDA_TENSOR16_RESOURCE, sizeof(CudaTensor16));
    if (!t)
        return NULL;
 ...
 */
ERL_NIF_TERM ct16_from_list(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
 ...
 */
ERL_NIF_TERM ct16_to_list(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    CudaTensor16 *t = get_cuda_tensor16(env, argv[0]);
    if (!t)
        return make_error(env, "invalid_cuda_tensor16");
...
ERL_NIF_TERM ct16_shape(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    CudaTensor16 *t = get_cuda_tensor16(env, argv[0]);
    if (!t)
        return make_error(env, "invalid_cuda_tensor16");
 ...
 */
ERL_NIF_TERM ct16_matmul(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    ...
    int result = cuda_hgemm_gpu(m, n, k, 1.0f, a->d_data, k, b->d_data, n, 0.0f, c->d_data, n);
 ...
 */
ERL_NIF_TERM ct16_matmul_inplace_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    CudaTensor16 *a = get_cuda_tensor16(env, argv[0]);
    CudaTensor16 *b = get_cuda_tensor16(env, argv[1]);
 ...
 */
ERL_NIF_TERM ct16_matmul_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    CudaTensor16 *a = get_cuda_tensor16(env, argv[0]);
    CudaTensor16 *b = get_cuda_tensor16(env, argv[1]);
    CudaTensor16 *c = get_cuda_tensor16(env, argv[2]);
    ...
    int m, n, k, iters;
    if (!enif_get_int(env, argv[3], &m) || !enif_get_int(env, argv[4], &n) ||
        !enif_get_int(env, argv[5], &k) || !enif_get_int(env, argv[6], &iters))
 ...
 */
ERL_NIF_TERM ct16_matmul_lt_32f_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    CudaTensor16 *a = get_cuda_tensor16(env, argv[0]);
    CudaTensor16 *b = get_cuda_tensor16(env, argv[1]);
    CudaTensor16 *c = get_cuda_tensor16(env, argv[2]);
 ...
 */
ERL_NIF_TERM ct16_matmul_fused_relu_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    CudaTensor16 *a = get_cuda_tensor16(env, argv[0]);
    CudaTensor16 *b = get_cuda_tensor16(env, argv[1]);
    CudaTensor16 *c = get_cuda_tensor16(env, argv[2]);
 ...
 */
ERL_NIF_TERM ct16_matmul_fused_gelu_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    CudaTensor16 *a = get_cuda_tensor16(env, argv[0]);
    CudaTensor16 *b = get_cuda_tensor16(env, argv[1]);
    CudaTensor16 *c = get_cuda_tensor16(env, argv[2]);
 ...
 */
ERL_NIF_TERM ct16_linear_relu_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    CudaTensor16 *a = get_cuda_tensor16(env, argv[0]);
    CudaTensor16 *b = get_cuda_tensor16(env, argv[1]);
    ...
    int result = cuda_hgemm_fused_relu_bias(m, n, k, a->d_data, b->d_data, bias->d_data, c->d_data);
    if (result != 0)
 ...
 */
ERL_NIF_TERM ct16_linear_gelu_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    CudaTensor16 *a = get_cuda_tensor16(env, argv[0]);
    CudaTensor16 *b = get_cuda_tensor16(env, argv[1]);
    CudaTensor16 *bias = get_cuda_tensor16(env, argv[2]);
    ...
    int result = cuda_hgemm_fused_gelu_bias(m, n, k, a->d_data, b->d_data, bias->d_data, c->d_data);
    if (result != 0)
 ...
 */
ERL_NIF_TERM ct16_matmul_fused_relu_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    CudaTensor16 *a = get_cuda_tensor16(env, argv[0]);
    CudaTensor16 *b = get_cuda_tensor16(env, argv[1]);
    CudaTensor16 *c = get_cuda_tensor16(env, argv[2]);
 ...
 */
ERL_NIF_TERM ct16_matmul_fused_gelu_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    CudaTensor16 *a = get_cuda_tensor16(env, argv[0]);
    CudaTensor16 *b = get_cuda_tensor16(env, argv[1]);
    CudaTensor16 *c = get_cuda_tensor16(env, argv[2]);
 ...
 */
ERL_NIF_TERM ct16_matmul_batched_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int m, n, k, batch_count, iters;
    if (!enif_get_int(env, argv[0], &m) || !enif_get_int(env, argv[1], &n) ||
        !enif_get_int(env, argv[2], &k) || !enif_get_int(env, argv[3], &batch_count) ||
 ...
 */
ERL_NIF_TERM fp8_matmul_lt_tn_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int m, n, k, iters;
    if (!enif_get_int(env, argv[0], &m) || !enif_get_int(env, argv[1], &n) ||
        !enif_get_int(env, argv[2], &k) || !enif_get_int(env, argv[3], &iters))
        return make_error(env, "invalid_args");
 ...
 */
ERL_NIF_TERM cutlass_fp8_f16acc_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int m, n, k, iters;
    if (!enif_get_int(env, argv[0], &m) || !enif_get_int(env, argv[1], &n) ||
        !enif_get_int(env, argv[2], &k) || !enif_get_int(env, argv[3], &iters))
 ...
 */
ERL_NIF_TERM cutlass_fp8_f32acc_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int m, n, k, iters;
    if (!enif_get_int(env, argv[0], &m) || !enif_get_int(env, argv[1], &n) ||
        !enif_get_int(env, argv[2], &k) || !enif_get_int(env, argv[3], &iters))
...
ERL_NIF_TERM cuda_axpy_loop_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int n, iters;
    if (!enif_get_int(env, argv[0], &n) || !enif_get_int(env, argv[1], &iters))
        return make_error(env, "invalid_args");
...
ERL_NIF_TERM cuda_axpy_graph_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int n, iters;
    if (!enif_get_int(env, argv[0], &n) || !enif_get_int(env, argv[1], &iters))
        return make_error(env, "invalid_args");
...
ERL_NIF_TERM cublaslt_fp8_algo_sweep_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
...
ERL_NIF_TERM nvfp4_fused_gemm_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int m, n, k, iters;
    if (!enif_get_int(env, argv[0], &m) || !enif_get_int(env, argv[1], &n) ||
        !enif_get_int(env, argv[2], &k) || !enif_get_int(env, argv[3], &iters))
        return make_error(env, "invalid_args");
    int us = nvfp4_fused_gemm_bench(m, n, k, iters);
    if (us < 0) {
        char errbuf[64];
        snprintf(errbuf, sizeof(errbuf), "nvfp4_fused_gemm_failed_%d", us);
        return make_error(env, errbuf);
    }
    return enif_make_tuple2(env, enif_make_atom(env, "ok"), enif_make_int(env, us));
}

/** cutlass_fp8_serial_bench_nif(M, N, K, Iters) -> {ok, ElapsedUs} */
ERL_NIF_TERM cutlass_fp8_serial_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int m, n, k, iters;
    if (!enif_get_int(env, argv[0], &m) || !enif_get_int(env, argv[1], &n) ||
        !enif_get_int(env, argv[2], &k) || !enif_get_int(env, argv[3], &iters))
        return make_error(env, "invalid_args");
    int us = cutlass_fp8_serial_bench(m, n, k, iters);
    if (us < 0) {
        char errbuf[64];
        snprintf(errbuf, sizeof(errbuf), "cutlass_fp8_serial_failed_%d", us);
        return make_error(env, errbuf);
    }
    return enif_make_tuple2(env, enif_make_atom(env, "ok"), enif_make_int(env, us));
}

/** cutlass_fp8_concurrent_bench_nif(M, N, K, Iters) -> {ok, ElapsedUs} */
ERL_NIF_TERM cutlass_fp8_concurrent_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int m, n, k, iters;
    if (!enif_get_int(env, argv[0], &m) || !enif_get_int(env, argv[1], &n) ||
        !enif_get_int(env, argv[2], &k) || !enif_get_int(env, argv[3], &iters))
        return make_error(env, "invalid_args");
    int us = cutlass_fp8_concurrent_bench(m, n, k, iters);
    if (us < 0) {
        char errbuf[64];
        snprintf(errbuf, sizeof(errbuf), "cutlass_fp8_concurrent_failed_%d", us);
        return make_error(env, errbuf);
    }
    return enif_make_tuple2(env, enif_make_atom(env, "ok"), enif_make_int(env, us));
}

/** cublaslt_fp16_algo_sweep_nif(M, N, K, Iters, MaxAlgos) -> {ok, BestUs} */
ERL_NIF_TERM cublaslt_fp16_algo_sweep_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int m, n, k, iters, max_algos;
    if (!enif_get_int(env, argv[0], &m) || !enif_get_int(env, argv[1], &n) ||
        !enif_get_int(env, argv[2], &k) || !enif_get_int(env, argv[3], &iters) ||
        !enif_get_int(env, argv[4], &max_algos))
        return make_error(env, "invalid_args");
    int us = cublaslt_fp16_algo_sweep(m, n, k, iters, max_algos);
    if (us < 0) {
        char errbuf[64];
        snprintf(errbuf, sizeof(errbuf), "cublaslt_fp16_algo_sweep_failed_%d", us);
        return make_error(env, errbuf);
    }
    return enif_make_tuple2(env, enif_make_atom(env, "ok"), enif_make_int(env, us));
}
...
ERL_NIF_TERM nvfp4_dequant_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int n, iters;
    if (!enif_get_int(env, argv[0], &n) || !enif_get_int(env, argv[1], &iters))
        return make_error(env, "invalid_args");
...
ERL_NIF_TERM cublaslt_fp16_fused_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int m, n, k, iters, ep;
    if (!enif_get_int(env, argv[0], &m) || !enif_get_int(env, argv[1], &n) ||
        !enif_get_int(env, argv[2], &k) || !enif_get_int(env, argv[3], &iters) ||
 ...
 */
ERL_NIF_TERM cublaslt_fp16_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int m, n, k, iters;
    if (!enif_get_int(env, argv[0], &m) || !enif_get_int(env, argv[1], &n) ||
        !enif_get_int(env, argv[2], &k) || !enif_get_int(env, argv[3], &iters))
 ...
 */
ERL_NIF_TERM cutlass_fp8_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
 ...
 */
ERL_NIF_TERM cutlass_int8_sparse_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    ...
    switch (config) {
    case 0:
        result = cutlass_int8_sparse_gemm_bench(m, n, k, iters);
        break;
    case 1:
        result = cutlass_int8_sparse_gemm_bench_b(m, n, k, iters);
        break;
    case 2:
        result = cutlass_int8_sparse_gemm_bench_c(m, n, k, iters);
        break;
    case 3:
        result = cutlass_int8_sparse_gemm_bench_d(m, n, k, iters);
        break;
    default:
        return make_error(env, "invalid_config");
    }
 ...
 */
ERL_NIF_TERM cutlass_int8_sparse_bench_ex_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int m, n, k, iters, config, split_k;
    if (!enif_get_int(env, argv[0], &m) || !enif_get_int(env, argv[1], &n) ||
        !enif_get_int(env, argv[2], &k) || !enif_get_int(env, argv[3], &iters) ||
        !enif_get_int(env, argv[4], &config) || !enif_get_int(env, argv[5], &split_k))
        return make_error(env, "invalid_args");
 ...
 */
ERL_NIF_TERM cusparselt_int8_sparse_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int m, n, k, iters, mode;
    if (!enif_get_int(env, argv[0], &m) || !enif_get_int(env, argv[1], &n) ||
        !enif_get_int(env, argv[2], &k) || !enif_get_int(env, argv[3], &iters) ||
 ...
 */
ERL_NIF_TERM cusparselt_fp8_sparse_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int m, n, k, iters;
    if (!enif_get_int(env, argv[0], &m) || !enif_get_int(env, argv[1], &n) ||
        !enif_get_int(env, argv[2], &k) || !enif_get_int(env, argv[3], &iters))
 ...
 */
ERL_NIF_TERM cusparselt_fp16_sparse_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int m, n, k, iters;
    if (!enif_get_int(env, argv[0], &m) || !enif_get_int(env, argv[1], &n) ||
        !enif_get_int(env, argv[2], &k) || !enif_get_int(env, argv[3], &iters))
 ...
 */
ERL_NIF_TERM cutlass_int4_sparse_bench_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int m, n, k, iters, config, split_k;
    if (!enif_get_int(env, argv[0], &m) || !enif_get_int(env, argv[1], &n) ||
        !enif_get_int(env, argv[2], &k) || !enif_get_int(env, argv[3], &iters) ||
...
ERL_NIF_TERM ct16_matmul_fused_relu_tn_bench_nif(ErlNifEnv *env, int argc,
    ...
    if (!a || !b || !c)
        return make_error(env, "invalid_cuda_tensor16");
    if (!b->d_data_t)
        return make_error(env, "b_not_transposed");
...
ERL_NIF_TERM ct16_available(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    (void)argv;
    return cuda_fp16_available() ? enif_make_atom(env, "true") : enif_make_atom(env, "false");
}

/** cuda_sync() -> ok
 *  Explicit GPU sync - call when you need results
 */
ERL_NIF_TERM nif_cuda_sync(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    (void)argv;
    cuda_explicit_sync();
    return make_ok_nil(env);
 ...
 */
ERL_NIF_TERM ct16_matmul_async(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    CudaTensor16 *a = get_cuda_tensor16(env, argv[0]);
    CudaTensor16 *b = get_cuda_tensor16(env, argv[1]);
    if (!a || !b)
    ...
    /* HGEMM async - no sync! */
    int result =
        cuda_hgemm_gpu_async(m, n, k, 1.0f, a->d_data, k, b->d_data, n, 0.0f, c->d_data, n);
