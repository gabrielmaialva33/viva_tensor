...
template <
    ...
    const int
        thread_m_blocks, // number of 16x16 blocks in the m dimension (batchsize) of the threadblock
    const int thread_n_blocks, // same for n dimension (output)
    const int thread_k_blocks, // same for k dimension (reduction)
    const int stages,          // number of stages for the async global->shared fetch pipeline
    const int group_blocks =
        -1 // number of consecutive 16x16 blocks with a separate quantization scale
    >
__global__ void Marlin(
    ...
    constexpr int a_gl_rd_delta_o =
        16 * thread_k_blocks / 8; // delta between subsequent A tiles in global memory
    int a_gl_rd_delta_i =
        a_gl_stride * (threads / a_gl_rd_delta_o); // between subsequent accesses within a tile
    constexpr int a_sh_wr_delta =
        a_sh_stride * (threads / a_gl_rd_delta_o); // between shared memory writes
    constexpr int a_sh_rd_delta_o =
        2 * ((threads / 32) / (thread_n_blocks / 4)); // between shared memory tile reads
    ...
    constexpr int a_sh_wr_iters =
        ceildiv(a_sh_stage, a_sh_wr_delta); // number of shared write iterations for a tile
...
#pragma unroll
    ...
    auto thread_block_reduce = [&]() {
        ...
        if (red_off >= 1) {
            ...
            int red_sh_rd =
                red_sh_stride * (threadIdx.x / b_sh_stride) + (threadIdx.x % b_sh_stride);
...
#pragma unroll
        ...
        if (slice_iters == 0) {
            ...
            if (slice_count >
                1) { // only globally reduce if there is more than one block in a slice
            ...
            if (slice_iters) {
                a_gl_rd =
                    a_gl_stride * (threadIdx.x / a_gl_rd_delta_o) + (threadIdx.x % a_gl_rd_delta_o);
#pragma unroll
                for (int i = 0; i < b_sh_wr_iters; i++)
...
#define CALL_IF(THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS)                  \
        ...
        cudaFuncSetAttribute(Marlin<THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS,   \
                                    STAGES, GROUP_BLOCKS>,                                        \
        ...
        Marlin<THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS>  \
            <<<blocks, THREADS, SHARED_MEM, stream>>>(A_ptr, B_ptr, C_ptr, s_ptr, prob_m, prob_n, \
                                                      prob_k, locks);                             \
    }
...
int marlin_cuda(const void *A, const void *B, void *C, void *s, int prob_m, int prob_n, int prob_k,
