...
int cusparselt_layout_test(int M, int N, int K, int iters, int layout) {
    ...
    switch (layout) {
    case 0: /* ROW/COL NN */
        orderA = CUSPARSE_ORDER_ROW;
        orderB = CUSPARSE_ORDER_COL;
        orderC = CUSPARSE_ORDER_ROW;
        ...
        break;
    case 1: /* COL/COL TN */
        orderA = CUSPARSE_ORDER_COL;
        orderB = CUSPARSE_ORDER_COL;
        orderC = CUSPARSE_ORDER_COL;
    ...
    st = cusparseLtStructuredDescriptorInit(&handle, &matA,
                                            (opA == CUSPARSE_OPERATION_NON_TRANSPOSE ? M : K),
    ...
    st = cusparseLtDenseDescriptorInit(&handle, &matB,
                                       (opB == CUSPARSE_OPERATION_NON_TRANSPOSE ? K : N),
                                       (opB == CUSPARSE_OPERATION_NON_TRANSPOSE ? N : K), ldB, 16,
                                       CUDA_R_8I, orderB);
    if (st != CUSPARSE_STATUS_SUCCESS) {
    ...
    st = cusparseLtDenseDescriptorInit(&handle, &matC, M, N, ldC, 16, CUDA_R_32I, orderC);
    if (st != CUSPARSE_STATUS_SUCCESS) {
    ...
    st = cusparseLtDenseDescriptorInit(&handle, &matD, M, N, ldC, 16, CUDA_R_32I, orderC);
    if (st != CUSPARSE_STATUS_SUCCESS) {
    ...
    st = cusparseLtMatmulDescriptorInit(&handle, &matmulDescr, opA, opB, &matA, &matB, &matC, &matD,
                                        CUSPARSE_COMPUTE_32I);
    if (st != CUSPARSE_STATUS_SUCCESS) {
        fprintf(stderr, "[layout %d %s] matmul descr init failed: %d\n", layout,
                layoutNames[layout], st);
        cusparseLtDestroy(&handle);
        return -7;
    }

    st = cusparseLtMatmulAlgSelectionInit(&handle, &algSel, &matmulDescr,
    ...
    /* Search */
    st = cusparseLtMatmulSearch(&handle, &plan, &alpha, d_compressed, d_B, &beta, d_C, d_C,
...

    {
        ...
        for (int i = 0; i < iters; i++) {
            cusparseLtMatmul(&handle, &plan, &alpha, d_compressed, d_B, &beta, d_C, d_C,
                             d_workspace, nullptr, 0);
        }
        ...
        fprintf(stderr, "[layout %d %s] %d algos, %.2f ms, %.1f TOPS (%.1f%% of 1321T)\n", layout,
                layoutNames[layout], maxAlgId, ms, tops, tops / 1321 * 100);
