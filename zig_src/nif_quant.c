 ...
 */
ERL_NIF_TERM nt_matmul_int8(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 6)
...
ERL_NIF_TERM nt_quantize_int8(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 1)
        return enif_make_badarg(env);
 ...
 */
ERL_NIF_TERM nt_matmul_nf4(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 7)
        return enif_make_badarg(env);
    ...
    if (!enif_get_int(env, argv[3], &m) || !enif_get_int(env, argv[4], &n) ||
        !enif_get_int(env, argv[5], &k) || !enif_get_int(env, argv[6], &block_size))
        return enif_make_badarg(env);
...
ERL_NIF_TERM nt_to_qint8(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 1)
        return enif_make_badarg(env);
    ...
    QuantInt8Tensor *q =
        (QuantInt8Tensor *)enif_alloc_resource(QINT8_RESOURCE, sizeof(QuantInt8Tensor));
    if (!q)
        return make_error(env, "alloc_failed");
 ...
 *  Dequant B + MKL DGEMM.
 */
ERL_NIF_TERM nt_matmul_qint8(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 5)
        return enif_make_badarg(env);
    ...
    cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, m, n, k, 1.0, a->data, k, b_dequant, n,
                0.0, c->data, n);
...
ERL_NIF_TERM nt_to_qnf4(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 2)
        return enif_make_badarg(env);
    ...
    QuantNF4Tensor *q =
        (QuantNF4Tensor *)enif_alloc_resource(QNF4_RESOURCE, sizeof(QuantNF4Tensor));
    if (!q)
        return make_error(env, "alloc_failed");
...
static const double NF4_LEVELS[16] = {-1.0,
                                      -0.6961928009986877,
                                      -0.5250730514526367,
                                      -0.39491748809814453,
                                      -0.28444138169288635,
 ...
 */
ERL_NIF_TERM nt_matmul_qnf4(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 5)
        return enif_make_badarg(env);
...
ERL_NIF_TERM qint8_scale(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 1)
        return enif_make_badarg(env);
...
ERL_NIF_TERM qint8_shape(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 1)
        return enif_make_badarg(env);
    QuantInt8Tensor *q = get_qint8(env, argv[0]);
    if (!q)
    ...
    ERL_NIF_TERM shape[2] = {enif_make_int(env, q->shape[0]), enif_make_int(env, q->shape[1])};
    return make_ok(env, enif_make_list_from_array(env, shape, 2));
}

/** qnf4_info(QuantNF4Tensor) -> {ok, #{block_size, num_blocks, compression}} */
ERL_NIF_TERM qnf4_info(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 1)
        return enif_make_badarg(env);
    ...
    double compression =
        (double)(q->size * sizeof(double)) / (q->packed_size + q->num_blocks * sizeof(double));

    ERL_NIF_TERM keys[] = {enif_make_atom(env, "block_size"), enif_make_atom(env, "num_blocks"),
                           enif_make_atom(env, "compression")};
    ERL_NIF_TERM vals[] = {enif_make_int(env, q->block_size), enif_make_int(env, q->num_blocks),
