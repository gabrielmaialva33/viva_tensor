...
int cuda_init(void) {
    ...
    g_cublas_set_workspace =
        (cublas_set_workspace_fn)dlsym(g_cublas_handle, "cublasSetWorkspace_v2");
    g_cublas_gemm_strided_batched_ex =
        (cublas_gemm_strided_batched_ex_fn)dlsym(g_cublas_handle, "cublasGemmStridedBatchedEx");
    ...
    if (g_cublas_set_workspace && g_cuda_malloc) {
        ...
        if (werr == cudaSuccess && g_cublas_workspace) {
            cublasStatus_t wstat =
                g_cublas_set_workspace(g_cublas_ctx, g_cublas_workspace, CUBLAS_WORKSPACE_SIZE);
    ...
    const char *tensor_core_status =
        g_cublas_gemm_ex ? "INT8/FP16 Tensor Cores ENABLED" : "FP32 only";
 ...
 */
int cuda_dgemm(int M, int N, int K, double alpha, const double *A, int lda, const double *B,
    ...
    stat = g_cublas_dgemm(g_cublas_ctx, CUBLAS_OP_N,
                          CUBLAS_OP_N,    /* No transpose (we're doing the swap trick) */
                          N, M, K,        /* Swapped dimensions for row-major */
                          &alpha, d_B, N, /* B with leading dim N */
                          d_A, K,         /* A with leading dim K */
                          &beta, d_C, N   /* C with leading dim N */
    );
 ...
 * A is M x K, B is K x N, C is M x N (all row-major)
 */
int cuda_sgemm(int M, int N, int K, float alpha, const float *A, int lda, const float *B, int ldb,
               float beta, float *C, int ldc) {
    if (!g_cuda_available || !g_cublas_sgemm)
        return -1;
    ...
    stat = g_cublas_sgemm(g_cublas_ctx, CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &alpha, d_B, N, d_A, K,
                          &beta, d_C, N);
...
int cuda_tensor_upload(float *d_dst, const float *h_src, size_t num_elements) {
    ...
    cudaError_t err =
        g_cuda_memcpy(d_dst, h_src, num_elements * sizeof(float), cudaMemcpyHostToDevice);
...
int cuda_tensor_download(float *h_dst, const float *d_src, size_t num_elements) {
    ...
    cudaError_t err =
        g_cuda_memcpy(h_dst, d_src, num_elements * sizeof(float), cudaMemcpyDeviceToHost);
    return (err == cudaSuccess) ? 0 : -1;
}
 ...
 */
int cuda_sgemm_gpu(int M, int N, int K, float alpha, const float *d_A, int lda, const float *d_B,
                   int ldb, float beta, float *d_C, int ldc) {
    if (!g_cuda_available)
        return -1;
    ...
    if (!g_cublas_sgemm)
        return -1;
    cublasStatus_t stat = g_cublas_sgemm(g_cublas_ctx, CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &alpha,
 ...
 */
int cuda_tensor_upload_batch(float **d_dsts, const float **h_srcs, const size_t *sizes, int count) {
...
int cuda_igemm_lt_gpu(int M, int N, int K, const int8_t *d_A, int lda, const int8_t *d_B, int ldb,
                      int32_t *d_C, int ldc);
...
int cuda_tensor_upload_int8(int8_t *d_dst, const int8_t *h_src, size_t num_elements) {
    ...
    cudaError_t err =
        g_cuda_memcpy(d_dst, h_src, num_elements * sizeof(int8_t), cudaMemcpyHostToDevice);
    return (err == cudaSuccess) ? 0 : -1;
}
...
int cuda_tensor_download_int32(int32_t *h_dst, const int32_t *d_src, size_t num_elements) {
    ...
    cudaError_t err =
        g_cuda_memcpy(h_dst, d_src, num_elements * sizeof(int32_t), cudaMemcpyDeviceToHost);
    return (err == cudaSuccess) ? 0 : -1;
}
 ...
 */
int cuda_igemm(int M, int N, int K, int32_t alpha, const int8_t *A, int lda, const int8_t *B,
...
int cuda_hgemm_gpu(int M, int N, int K, float alpha, const cuda_half_t *d_A, int lda,
    ...
    cublasStatus_t stat =
        g_cublas_gemm_ex(g_cublas_ctx, CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &alpha, d_B, CUDA_R_16F,
                         N,                           /* B is FP16 */
 ...
 */
int cuda_hgemm_gpu_pure16(int M, int N, int K, const cuda_half_t *d_A, int lda,
                          const cuda_half_t *d_B, int ldb, cuda_half_t *d_C, int ldc) {
    if (!g_cuda_available || !g_cublas_gemm_ex)
        return -1;
    ...
    cublasStatus_t stat =
        g_cublas_gemm_ex(g_cublas_ctx, CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &alpha_h, d_B, CUDA_R_16F,
                         N, d_A, CUDA_R_16F, K, &beta_h, d_C, CUDA_R_16F,
                         N,                  /* Output FP16, half the bandwidth */
 ...
 */
int cuda_hgemm_gpu_pure16_async(int M, int N, int K, const cuda_half_t *d_A, int lda,
                                const cuda_half_t *d_B, int ldb, cuda_half_t *d_C, int ldc) {
    if (!g_cuda_available || !g_cublas_gemm_ex)
        return -1;
    ...
    cublasStatus_t stat =
        g_cublas_gemm_ex(g_cublas_ctx, CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &alpha_h, d_B, CUDA_R_16F,
                         N, d_A, CUDA_R_16F, K, &beta_h, d_C, CUDA_R_16F, N, CUBLAS_COMPUTE_16F,
                         CUBLAS_GEMM_DEFAULT_TENSOR_OP);
 ...
 */
int cuda_sgemm_gpu_inplace(int M, int N, int K, float alpha, const float *d_A, int lda,
...
int cuda_tensor_upload_fp16(cuda_half_t *d_dst, const cuda_half_t *h_src, size_t num_elements) {
    ...
    cudaError_t err =
        g_cuda_memcpy(d_dst, h_src, num_elements * sizeof(cuda_half_t), cudaMemcpyHostToDevice);
    return (err == cudaSuccess) ? 0 : -1;
}
...
int cuda_tensor_download_fp16(cuda_half_t *h_dst, const cuda_half_t *d_src, size_t num_elements) {
    ...
    cudaError_t err =
        g_cuda_memcpy(h_dst, d_src, num_elements * sizeof(cuda_half_t), cudaMemcpyDeviceToHost);
    return (err == cudaSuccess) ? 0 : -1;
}
...
typedef cublasStatus_t (*cublaslt_matmul_desc_create_fn)(cublasLtMatmulDesc_t *,
                                                         cublasComputeType_t, cudaDataType_t);
typedef cublasStatus_t (*cublaslt_matmul_desc_destroy_fn)(cublasLtMatmulDesc_t);
typedef cublasStatus_t (*cublaslt_matmul_desc_set_attr_fn)(cublasLtMatmulDesc_t, int, const void *,
                                                           size_t);
typedef cublasStatus_t (*cublaslt_matrix_layout_create_fn)(cublasLtMatrixLayout_t *, cudaDataType_t,
...
typedef cublasStatus_t (*cublaslt_matmul_preference_set_attr_fn)(cublasLtMatmulPreference_t, int,
                                                                 const void *, size_t);
typedef cublasStatus_t (*cublaslt_matrix_layout_set_attr_fn)(cublasLtMatrixLayout_t, int,
                                                             const void *, size_t);
typedef cublasStatus_t (*cublaslt_matmul_algo_get_heuristic_fn)(
    ...
    cublasLtMatrixLayout_t, cublasLtMatrixLayout_t, cublasLtMatmulPreference_t, int,
    cublasLtHeuristicResult_t *, int *);
typedef cublasStatus_t (*cublaslt_matmul_fn)(cublasLtHandle_t, cublasLtMatmulDesc_t, const void *,
...
int cublaslt_init(void) {
    ...
    g_cublaslt_matmul_desc_create =
        (cublaslt_matmul_desc_create_fn)dlsym(g_cublaslt_handle, "cublasLtMatmulDescCreate");
    g_cublaslt_matmul_desc_destroy =
        (cublaslt_matmul_desc_destroy_fn)dlsym(g_cublaslt_handle, "cublasLtMatmulDescDestroy");
    g_cublaslt_matmul_desc_set_attr =
        (cublaslt_matmul_desc_set_attr_fn)dlsym(g_cublaslt_handle,
                                                "cublasLtMatmulDescSetAttribute");
    g_cublaslt_matrix_layout_create =
        (cublaslt_matrix_layout_create_fn)dlsym(g_cublaslt_handle, "cublasLtMatrixLayoutCreate");
    g_cublaslt_matrix_layout_destroy =
        (cublaslt_matrix_layout_destroy_fn)dlsym(g_cublaslt_handle, "cublasLtMatrixLayoutDestroy");
    g_cublaslt_matmul_preference_create =
        (cublaslt_matmul_preference_create_fn)dlsym(g_cublaslt_handle,
                                                    "cublasLtMatmulPreferenceCreate");
    g_cublaslt_matmul_preference_destroy =
        (cublaslt_matmul_preference_destroy_fn)dlsym(g_cublaslt_handle,
                                                     "cublasLtMatmulPreferenceDestroy");
    g_cublaslt_matmul_preference_set_attr =
        (cublaslt_matmul_preference_set_attr_fn)dlsym(g_cublaslt_handle,
                                                      "cublasLtMatmulPreferenceSetAttribute");
    g_cublaslt_matrix_layout_set_attr =
        (cublaslt_matrix_layout_set_attr_fn)dlsym(g_cublaslt_handle,
                                                  "cublasLtMatrixLayoutSetAttribute");
    g_cublaslt_matmul_algo_get_heuristic =
        (cublaslt_matmul_algo_get_heuristic_fn)dlsym(g_cublaslt_handle,
 ...
 */
int cuda_igemm_lt(int M, int N, int K, float alpha, const int8_t *A, int lda, const int8_t *B,
                  int ldb, float beta, int32_t *C, int ldc) {
 ...
 */
int cuda_hgemm_gpu_async(int M, int N, int K, float alpha, const cuda_half_t *d_A, int lda,
                         const cuda_half_t *d_B, int ldb, float beta, float *d_C, int ldc) {
    if (!g_cuda_available || !g_cublas_gemm_ex)
        return -1;
    ...
    cublasStatus_t stat =
        g_cublas_gemm_ex(g_cublas_ctx, CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &alpha, d_B, CUDA_R_16F,
                         N, d_A, CUDA_R_16F, K, &beta, d_C, CUDA_R_32F, N,
 ...
 */
int cuda_igemm_lt_gpu_tn(int M, int N, int K, const int8_t *d_A, const int8_t *d_B_T,
                         int32_t *d_C) {
    if (g_cublaslt_available < 0)
        cublaslt_init();
    if (!g_cublaslt_available || !g_cublaslt_matmul)
        return -1;
    ...
    int need_heuristic =
        !g_int8_cache_valid || g_int8_cache_m != M || g_int8_cache_n != N || g_int8_cache_k != K;
    ...
    if (need_heuristic) {
        ...
        g_cublaslt_matmul_desc_set_attr(matmul_desc, CUBLASLT_MATMUL_DESC_TRANSA, &op_t,
                                        sizeof(op_t));
        g_cublaslt_matmul_desc_set_attr(matmul_desc, CUBLASLT_MATMUL_DESC_TRANSB, &op_n,
                                        sizeof(op_n));
        ...
        g_cublaslt_matrix_layout_create(&layout_bt, CUDA_R_8I, (uint64_t)K, (uint64_t)N,
                                        (int64_t)K);
        ...
        g_cublaslt_matrix_layout_create(&layout_c, CUDA_R_32I, (uint64_t)N, (uint64_t)M,
                                        (int64_t)N);
        ...
        stat = g_cublaslt_matmul_algo_get_heuristic(g_cublaslt_ctx, matmul_desc, layout_bt,
                                                    layout_a, layout_c, layout_c, preference, 8,
        ...
        stat = g_cublaslt_matmul(g_cublaslt_ctx, matmul_desc, &alpha_i, d_B_T,
                                 layout_bt,     /* B_T: first operand (will be transposed) */
                                 d_A, layout_a, /* A: second operand */
                                 &beta_i, d_C, layout_c, d_C, layout_c, &results[0].algo,
                                 g_cublaslt_workspace, ws_size, (cudaStream_t)0);
        ...
        stat =
            g_cublaslt_matmul(g_cublaslt_ctx, matmul_desc, &alpha_i, d_B_T, layout_bt, d_A,
                              layout_a, &beta_i, d_C, layout_c, d_C, layout_c,
                              &g_int8_cached_result.algo, g_cublaslt_workspace,
                              g_cublaslt_workspace ? CUBLASLT_WORKSPACE_SIZE : 0, (cudaStream_t)0);
...
int cuda_hgemm_lt_gpu_tn(int M, int N, int K, const cuda_half_t *d_A, const cuda_half_t *d_B_T,
    ...
    int need_heuristic =
        !g_fp16_cache_valid || g_fp16_cache_m != M || g_fp16_cache_n != N || g_fp16_cache_k != K;
    ...
    if (need_heuristic) {
        ...
        cublasLtMatrixLayout_t layout_bt, layout_a, layout_c;
        g_cublaslt_matrix_layout_create(&layout_bt, CUDA_R_16F, (uint64_t)K, (uint64_t)N,
                                        (int64_t)K);
        g_cublaslt_matrix_layout_create(&layout_a, CUDA_R_16F, (uint64_t)K, (uint64_t)M,
                                        (int64_t)K);
        g_cublaslt_matrix_layout_create(&layout_c, CUDA_R_16F, (uint64_t)N, (uint64_t)M,
                                        (int64_t)N);
        ...
        stat = g_cublaslt_matmul(g_cublaslt_ctx, matmul_desc, &alpha_h, d_B_T, layout_bt, d_A,
                                 layout_a, &beta_h, d_C, layout_c, d_C, layout_c, &results[0].algo,
                                 g_cublaslt_workspace, ws_size, (cudaStream_t)0);
        ...
        stat =
            g_cublaslt_matmul(g_cublaslt_ctx, matmul_desc, &alpha_h, d_B_T, layout_bt, d_A,
                              layout_a, &beta_h, d_C, layout_c, d_C, layout_c,
                              &g_fp16_cached_result.algo, g_cublaslt_workspace,
                              g_cublaslt_workspace ? CUBLASLT_WORKSPACE_SIZE : 0, (cudaStream_t)0);
...
int cuda_igemm_lt_gpu(int M, int N, int K, const int8_t *d_A, int lda, const int8_t *d_B, int ldb,
    ...
    int32_t alpha_i = 1, beta_i = 0;
    cublasStatus_t stat =
        g_cublas_gemm_ex(g_cublas_ctx, CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &alpha_i, d_B, CUDA_R_8I,
                         N, d_A, CUDA_R_8I, K, &beta_i, d_C, CUDA_R_32I, N, CUBLAS_COMPUTE_32I,
                         CUBLAS_GEMM_DEFAULT);
 ...
 */
int cuda_igemm_lt_gpu_async(int M, int N, int K, const int8_t *d_A, int lda, const int8_t *d_B,
    ...
    if (!g_int8_cache_valid || g_int8_cache_m != M || g_int8_cache_n != N || g_int8_cache_k != K) {
        return cuda_igemm_lt_gpu(M, N, K, d_A, lda, d_B, ldb, d_C, ldc);
    }
    ...
    g_cublaslt_matmul_desc_set_attr(matmul_desc, CUBLASLT_MATMUL_DESC_TRANSA, &op_n, sizeof(op_n));
    g_cublaslt_matmul_desc_set_attr(matmul_desc, CUBLASLT_MATMUL_DESC_TRANSB, &op_n, sizeof(op_n));
    ...
    if (g_int8_cache_valid == 1) {
        ...
        g_cublaslt_matrix_layout_create(&layout_c, CUDA_R_32I, (uint64_t)M, (uint64_t)N,
                                        (int64_t)N);
        if (g_cublaslt_matrix_layout_set_attr) {
            g_cublaslt_matrix_layout_set_attr(layout_a, CUBLASLT_MATRIX_LAYOUT_ORDER, &row_order,
                                              sizeof(row_order));
            g_cublaslt_matrix_layout_set_attr(layout_b, CUBLASLT_MATRIX_LAYOUT_ORDER, &row_order,
                                              sizeof(row_order));
            g_cublaslt_matrix_layout_set_attr(layout_c, CUBLASLT_MATRIX_LAYOUT_ORDER, &row_order,
                                              sizeof(row_order));
        }
        stat =
            g_cublaslt_matmul(g_cublaslt_ctx, matmul_desc, &alpha, d_A, layout_a, d_B, layout_b,
                              &beta, d_C, layout_c, d_C, layout_c, &g_int8_cached_result.algo,
        ...
        g_cublaslt_matrix_layout_create(&layout_c, CUDA_R_32I, (uint64_t)N, (uint64_t)M,
                                        (int64_t)N);
        stat =
            g_cublaslt_matmul(g_cublaslt_ctx, matmul_desc, &alpha, d_B, layout_a, d_A, layout_b,
                              &beta, d_C, layout_c, d_C, layout_c, &g_int8_cached_result.algo,
                              g_cublaslt_workspace,
                              g_cublaslt_workspace ? CUBLASLT_WORKSPACE_SIZE : 0, (cudaStream_t)0);
    }
 ...
 */
static int cuda_hgemm_fused_internal(int M, int N, int K, const cuda_half_t *d_A,
                                     const cuda_half_t *d_B, cuda_half_t *d_C,
                                     const cuda_half_t *d_bias, int epilogue_type, int *cache_m,
                                     int *cache_n, int *cache_k,
                                     cublasLtHeuristicResult_t *cached_result, int *cache_valid) {
    if (g_cublaslt_available < 0)
        cublaslt_init();
    if (!g_cublaslt_available || !g_cublaslt_matmul)
        return -1;
    ...
    int need_heuristic = !(*cache_valid) || *cache_m != M || *cache_n != N || *cache_k != K;
    ...
    if (need_heuristic) {
        ...
        /* NN col-major swap: C^T = B^T @ A^T */
        cublasOperation_t op_n = CUBLAS_OP_N;
        g_cublaslt_matmul_desc_set_attr(matmul_desc, CUBLASLT_MATMUL_DESC_TRANSA, &op_n,
                                        sizeof(op_n));
        g_cublaslt_matmul_desc_set_attr(matmul_desc, CUBLASLT_MATMUL_DESC_TRANSB, &op_n,
                                        sizeof(op_n));
        ...
        g_cublaslt_matmul_desc_set_attr(matmul_desc, CUBLASLT_MATMUL_DESC_EPILOGUE, &epilogue,
                                        sizeof(epilogue));
        if (d_bias) {
            g_cublaslt_matmul_desc_set_attr(matmul_desc, CUBLASLT_MATMUL_DESC_BIAS_POINTER, &d_bias,
                                            sizeof(d_bias));
        }

        /* Layouts: col-major swap trick (swap A<->B, swap M<->N) */
        cublasLtMatrixLayout_t layout_b, layout_a, layout_c;
        g_cublaslt_matrix_layout_create(&layout_b, CUDA_R_16F, (uint64_t)N, (uint64_t)K,
                                        (int64_t)N);
        g_cublaslt_matrix_layout_create(&layout_a, CUDA_R_16F, (uint64_t)K, (uint64_t)M,
                                        (int64_t)K);
        g_cublaslt_matrix_layout_create(&layout_c, CUDA_R_16F, (uint64_t)N, (uint64_t)M,
        ...
        cublasLtHeuristicResult_t results[8];
        int returned_count = 0;
        stat = g_cublaslt_matmul_algo_get_heuristic(g_cublaslt_ctx, matmul_desc, layout_b, layout_a,
                                                    layout_c, layout_c, preference, 8, results,
                                                    &returned_count);

        if (stat != CUBLAS_STATUS_SUCCESS || returned_count == 0) {
            fprintf(stderr,
                    "[viva_tensor] FP16 fused epilogue=%d heuristic failed: stat=%d, count=%d\n",
        ...
        stat = g_cublaslt_matmul(g_cublaslt_ctx, matmul_desc, &alpha_f, d_B, layout_b, d_A,
                                 layout_a, /* swapped for col-major trick */
                                 &beta_f, d_C, layout_c, d_C, layout_c, &results[0].algo,
                                 g_cublaslt_workspace, ws_size, (cudaStream_t)0);
        ...
        *cache_valid = 1;
        ...
        stat =
            g_cublaslt_matmul(g_cublaslt_ctx, matmul_desc, &alpha_f, d_B, layout_b, d_A, layout_a,
                              &beta_f, d_C, layout_c, d_C, layout_c, &cached_result->algo,
                              g_cublaslt_workspace,
                              g_cublaslt_workspace ? CUBLASLT_WORKSPACE_SIZE : 0, (cudaStream_t)0);
...
int cuda_hgemm_lt_32f(int M, int N, int K, const cuda_half_t *d_A, const cuda_half_t *d_B,
                      cuda_half_t *d_C) {
    return cuda_hgemm_fused_internal(M, N, K, d_A, d_B, d_C, NULL, CUBLASLT_EPILOGUE_DEFAULT,
                                     &g_fp16_32f_cache_m, &g_fp16_32f_cache_n, &g_fp16_32f_cache_k,
                                     &g_fp16_32f_cached_result, &g_fp16_32f_cache_valid);
}
 ...
 */
static int cuda_hgemm_fused_tn_internal(int M, int N, int K, const cuda_half_t *d_A,
                                        const cuda_half_t *d_B_T, cuda_half_t *d_C,
                                        int epilogue_type, int *cache_m, int *cache_n, int *cache_k,
    ...
    if (need_heuristic) {
        ...
        if (stat != CUBLAS_STATUS_SUCCESS || returned_count == 0) {
            fprintf(stderr,
                    "[viva_tensor] FP16 fused TN epilogue=%d heuristic failed: stat=%d, count=%d\n",
                    epilogue_type, stat, returned_count);
            g_cublaslt_matmul_preference_destroy(preference);
            g_cublaslt_matrix_layout_destroy(layout_bt);
            g_cublaslt_matrix_layout_destroy(layout_a);
        ...
        stat = g_cublaslt_matmul(g_cublaslt_ctx, matmul_desc, &alpha_f, d_B_T, layout_bt, d_A,
                                 layout_a, &beta_f, d_C, layout_c, d_C, layout_c, &results[0].algo,
                                 g_cublaslt_workspace, ws_size, (cudaStream_t)0);
        ...
        *cache_valid = 1;
        ...
        stat =
            g_cublaslt_matmul(g_cublaslt_ctx, matmul_desc, &alpha_f, d_B_T, layout_bt, d_A,
                              layout_a, &beta_f, d_C, layout_c, d_C, layout_c, &cached_result->algo,
                              g_cublaslt_workspace,
                              g_cublaslt_workspace ? CUBLASLT_WORKSPACE_SIZE : 0, (cudaStream_t)0);
 ...
 */
int cuda_hgemm_fused_relu_tn(int M, int N, int K, const cuda_half_t *d_A, const cuda_half_t *d_B_T,
                             cuda_half_t *d_C) {
    return cuda_hgemm_fused_tn_internal(M, N, K, d_A, d_B_T, d_C, CUBLASLT_EPILOGUE_RELU,
 ...
 * d_B_T is pre-transposed B (N×K).
 */
int cuda_hgemm_fused_gelu_tn(int M, int N, int K, const cuda_half_t *d_A, const cuda_half_t *d_B_T,
                             cuda_half_t *d_C) {
    return cuda_hgemm_fused_tn_internal(M, N, K, d_A, d_B_T, d_C, CUBLASLT_EPILOGUE_GELU,
 ...
 */
int cuda_hgemm_fused_relu(int M, int N, int K, const cuda_half_t *d_A, const cuda_half_t *d_B,
                          cuda_half_t *d_C) {
    return cuda_hgemm_fused_internal(M, N, K, d_A, d_B, d_C, NULL, CUBLASLT_EPILOGUE_RELU,
 ...
 * Async, uses cached algorithm for repeated calls.
 */
int cuda_hgemm_fused_gelu(int M, int N, int K, const cuda_half_t *d_A, const cuda_half_t *d_B,
                          cuda_half_t *d_C) {
    return cuda_hgemm_fused_internal(M, N, K, d_A, d_B, d_C, NULL, CUBLASLT_EPILOGUE_GELU,
 ...
 */
int cuda_hgemm_fused_relu_bias(int M, int N, int K, const cuda_half_t *d_A, const cuda_half_t *d_B,
                               const cuda_half_t *d_bias, cuda_half_t *d_C) {
    return cuda_hgemm_fused_internal(M, N, K, d_A, d_B, d_C, d_bias, CUBLASLT_EPILOGUE_RELU_BIAS,
 ...
 * Bias is length N and is applied per output column.
 */
int cuda_hgemm_fused_gelu_bias(int M, int N, int K, const cuda_half_t *d_A, const cuda_half_t *d_B,
                               const cuda_half_t *d_bias, cuda_half_t *d_C) {
    return cuda_hgemm_fused_internal(M, N, K, d_A, d_B, d_C, d_bias, CUBLASLT_EPILOGUE_GELU_BIAS,
 ...
 */
int cuda_fp8gemm_lt_gpu_tn(int M, int N, int K, const uint8_t *d_A, const uint8_t *d_B_T,
                           uint16_t *d_C) {
    if (g_cublaslt_available < 0)
        cublaslt_init();
    if (!g_cublaslt_available || !g_cublaslt_matmul)
    ...
    int need_heuristic =
        !g_fp8_cache_valid || g_fp8_cache_m != M || g_fp8_cache_n != N || g_fp8_cache_k != K;
    ...
    if (need_heuristic) {
        ...
        cublasLtMatrixLayout_t layout_bt, layout_a, layout_c;
        g_cublaslt_matrix_layout_create(&layout_bt, CUDA_R_8F_E4M3, (uint64_t)K, (uint64_t)N,
                                        (int64_t)K);
        g_cublaslt_matrix_layout_create(&layout_a, CUDA_R_8F_E4M3, (uint64_t)K, (uint64_t)M,
                                        (int64_t)K);
        g_cublaslt_matrix_layout_create(&layout_c, CUDA_R_16F, (uint64_t)N, (uint64_t)M,
                                        (int64_t)N);
        ...
        stat =
            g_cublaslt_matmul(g_cublaslt_ctx, matmul_desc, &alpha_f, d_B_T, layout_bt, d_A,
                              layout_a, &beta_f, d_C, layout_c, d_C, layout_c,
                              &g_fp8_cached_result.algo, g_cublaslt_workspace,
                              g_cublaslt_workspace ? CUBLASLT_WORKSPACE_SIZE : 0, (cudaStream_t)0);
 ...
 */
int cuda_hgemm_batched(int M, int N, int K, int batch_count, const cuda_half_t *d_A,
                       const cuda_half_t *d_B, cuda_half_t *d_C) {
    if (!g_cuda_available || !g_cublas_gemm_strided_batched_ex)
        return -1;
    ...
    cublasStatus_t stat =
        g_cublas_gemm_strided_batched_ex(g_cublas_ctx, CUBLAS_OP_N, CUBLAS_OP_N, N, M,
                                         K, /* swapped M<->N for row-major */
                                         &alpha_h, d_B, CUDA_R_16F, N,
                                         strideB,                     /* B first (swapped) */
                                         d_A, CUDA_R_16F, K, strideA, /* A second (swapped) */
                                         &beta_h, d_C, CUDA_R_16F, N, strideC, batch_count,
