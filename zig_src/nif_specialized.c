...
ERL_NIF_TERM nt_resonance_mul(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
...
ERL_NIF_TERM nt_resonance_power(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    NativeTensor *a = get_tensor(env, argv[0]);
    if (!a)
        return make_error(env, "invalid_tensor");
...
LnsTensor *alloc_lns(int ndim, const int *shape) {
    LnsTensor *t = (LnsTensor *)enif_alloc_resource(LNS_RESOURCE, sizeof(LnsTensor));
    if (!t)
        return NULL;
    t->ndim = ndim;
    t->size = 1;
...
ERL_NIF_TERM lns_from_f64(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
 ...
 * Convert back to f64 */
ERL_NIF_TERM lns_to_f64(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
...
ERL_NIF_TERM lns_mul(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
...
ERL_NIF_TERM lns_mul_corrected(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    LnsTensor *a = get_lns(env, argv[0]);
    LnsTensor *b = get_lns(env, argv[1]);
    if (!a || !b)
...
ERL_NIF_TERM lns_div(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    LnsTensor *a = get_lns(env, argv[0]);
    LnsTensor *b = get_lns(env, argv[1]);
    if (!a || !b)
...
ERL_NIF_TERM lns_sqrt(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    LnsTensor *a = get_lns(env, argv[0]);
    if (!a)
        return make_error(env, "invalid_lns");
...
ERL_NIF_TERM lns_rsqrt(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    LnsTensor *a = get_lns(env, argv[0]);
    if (!a)
        return make_error(env, "invalid_lns");
...
ERL_NIF_TERM horde_create(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int count, dims;
    if (!enif_get_int(env, argv[0], &count) || !enif_get_int(env, argv[1], &dims) || count <= 0 ||
        dims < 1 || dims > 3)
...
ERL_NIF_TERM horde_set_positions(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
...
ERL_NIF_TERM horde_set_velocities(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    Horde *h = get_horde(env, argv[0]);
    if (!h)
        return make_error(env, "invalid_horde");
...
ERL_NIF_TERM horde_integrate_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    Horde *h = get_horde(env, argv[0]);
    if (!h)
        return make_error(env, "invalid_horde");
    ...
    vt_horde_integrate(h->positions, h->velocities, dt, h->entity_count * h->dims);
    return make_ok_nil(env);
}

/** horde_dampen(HordeRef, Friction) -> ok */
ERL_NIF_TERM horde_dampen_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    Horde *h = get_horde(env, argv[0]);
    if (!h)
        return make_error(env, "invalid_horde");
...
ERL_NIF_TERM horde_wrap_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    Horde *h = get_horde(env, argv[0]);
    if (!h)
        return make_error(env, "invalid_horde");
...
ERL_NIF_TERM horde_get_positions(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    Horde *h = get_horde(env, argv[0]);
    if (!h)
        return make_error(env, "invalid_horde");

    return make_ok(env, doubles_to_list(env, h->positions, h->entity_count * h->dims));
}

/** horde_get_velocities(HordeRef) -> {ok, List} */
ERL_NIF_TERM horde_get_velocities(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    Horde *h = get_horde(env, argv[0]);
    if (!h)
        return make_error(env, "invalid_horde");

    return make_ok(env, doubles_to_list(env, h->velocities, h->entity_count * h->dims));
}

/** horde_count(HordeRef) -> {ok, Int} */
ERL_NIF_TERM horde_count_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    Horde *h = get_horde(env, argv[0]);
    if (!h)
        return make_error(env, "invalid_horde");
...
ERL_NIF_TERM horde_kinetic_energy_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    Horde *h = get_horde(env, argv[0]);
    if (!h)
        return make_error(env, "invalid_horde");
...
ERL_NIF_TERM hdc_create_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int dim;
    if (!enif_get_int(env, argv[0], &dim) || dim <= 0 || dim % 64 != 0)
        return make_error(env, "dim_must_be_multiple_of_64");

    HdcVector *h = (HdcVector *)enif_alloc_resource(HDC_RESOURCE, sizeof(HdcVector));
    if (!h)
        return make_error(env, "out_of_memory");
...
ERL_NIF_TERM hdc_random_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    int dim;
    unsigned long seed;
    if (!enif_get_int(env, argv[0], &dim) || dim <= 0 || dim % 64 != 0)
...
ERL_NIF_TERM hdc_bind_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    ...
    HdcVector *c = (HdcVector *)enif_alloc_resource(HDC_RESOURCE, sizeof(HdcVector));
    if (!c)
        return make_error(env, "out_of_memory");
...
ERL_NIF_TERM hdc_similarity_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    HdcVector *a = get_hdc(env, argv[0]);
    HdcVector *b = get_hdc(env, argv[1]);
    if (!a || !b)
...
ERL_NIF_TERM hdc_permute_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
    HdcVector *a = get_hdc(env, argv[0]);
    if (!a)
        return make_error(env, "invalid_hdc");
...
/** hdc_dim(Hdc) -> {ok, Int} */
ERL_NIF_TERM hdc_dim_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void)argc;
