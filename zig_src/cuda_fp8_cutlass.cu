...
using GemmFP16Acc = cutlass::gemm::device::Gemm<
    ElementA_f16, LayoutA, ElementB_f16, LayoutB, ElementOut_f16, LayoutC,
    ...
    EpilogueOp_f16, cutlass::gemm::threadblock::GemmIdentityThreadblockSwizzle<>, 3, /* stages */
    16,                                   /* alignmentA */
    16,                                   /* alignmentB */
    false,                                /* SplitKSerial */
...
using EpilogueOp_f32 =
    cutlass::epilogue::thread::LinearCombination<ElementOut_f16, /* output type = half_t */
...
using Gemm_FP8_F32_LargeKN = cutlass::gemm::device::Gemm<
    ...
    cutlass::arch::OpClassTensorOp, cutlass::arch::Sm89, cutlass::gemm::GemmShape<128, 64, 128>,
    cutlass::gemm::GemmShape<64, 32, 128>, cutlass::gemm::GemmShape<16, 8, 32>, EpilogueOp_f32,
    cutlass::gemm::threadblock::GemmIdentityThreadblockSwizzle<>, kStages, 16, 16, false,
    cutlass::arch::OpMultiplyAdd /* standard FP32 accum */
    >;

using Gemm_FP8_F32_Default = cutlass::gemm::device::Gemm<
    ElementA_f16, LayoutA, ElementB_f16, LayoutB, ElementOut_f16, LayoutC, ElementAcc_f32,
    cutlass::arch::OpClassTensorOp, cutlass::arch::Sm89, cutlass::gemm::GemmShape<64, 128, 64>,
    cutlass::gemm::GemmShape<32, 64, 64>, cutlass::gemm::GemmShape<16, 8, 32>, EpilogueOp_f32,
    cutlass::gemm::threadblock::GemmIdentityThreadblockSwizzle<>, kStages, 16, 16, false,
...
using Gemm_FP8_F32_out_f32_LargeKN = cutlass::gemm::device::Gemm<
    ElementA_f16, LayoutA, ElementB_f16, LayoutB, float, LayoutC, ElementAcc_f32,
    cutlass::arch::OpClassTensorOp, cutlass::arch::Sm89, cutlass::gemm::GemmShape<128, 64, 128>,
    cutlass::gemm::GemmShape<64, 32, 128>, cutlass::gemm::GemmShape<16, 8, 32>,
    EpilogueOp_f32_out_f32, cutlass::gemm::threadblock::GemmIdentityThreadblockSwizzle<>, kStages,
    16, 16, false, cutlass::arch::OpMultiplyAdd>;
 ...
 */
__global__ void fp8_colmajor_dequant_to_fp16_kernel(const cutlass::float_e4m3_t *src,
                                                    const float *scales, cutlass::half_t *dst,
...
int cuda_fp8_colmajor_dequant_to_fp16(const void *d_fp8, const float *d_scales, void *d_fp16, int K,
                                      int N, void *stream) {
    /* Legacy per-channel entry — block_size = 0. */
    if (!d_fp8 || !d_fp16 || K <= 0 || N <= 0)
        return -10;
    int total = K * N;
    int threads = 256;
    int blocks = (total + threads - 1) / threads;
    fp8_colmajor_dequant_to_fp16_kernel<<<blocks, threads, 0, fp8_resolve_stream(stream)>>>(
        static_cast<const cutlass::float_e4m3_t *>(d_fp8), d_scales,
        static_cast<cutlass::half_t *>(d_fp16), K, N, 0);
    cudaError_t err = cudaGetLastError();
    return (err == cudaSuccess) ? 0 : -11;
}

/* Same but with per-block scales. block_size must divide K. */
int cuda_fp8_colmajor_dequant_to_fp16_blocked(const void *d_fp8, const float *d_scales,
    ...
    fp8_colmajor_dequant_to_fp16_kernel<<<blocks, threads, 0, fp8_resolve_stream(stream)>>>(
        static_cast<const cutlass::float_e4m3_t *>(d_fp8), d_scales,
        static_cast<cutlass::half_t *>(d_fp16), K, N, block_size);
    cudaError_t err = cudaGetLastError();
    return (err == cudaSuccess) ? 0 : -11;
}
 ...
 */
int cutlass_fp8_gemm_f32acc_out_f32(int M, int N, int K, const void *d_A, const void *d_B,
                                    float *d_C) {
    if (K == 4096 && N == 4096) {
    ...
    Gemm_FP8_F32_out_f32_Default::Arguments args({M, N, K},
                                                 {static_cast<const ElementA_f16 *>(d_A), K},
                                                 {static_cast<const ElementB_f16 *>(d_B), K},
                                                 {d_C, N}, {d_C, N}, {alpha, beta});
...
int cutlass_fp8_gemm_f16acc(int M, int N, int K, const void *d_A, const void *d_B, void *d_C) {
    ...
    GemmFP16Acc::Arguments args(
        {M, N, K}, {static_cast<const ElementA_f16 *>(d_A), K}, /* A: row-major, ld=K */
 ...
 * For comparison only.
 */
int cutlass_fp8_gemm_f32acc(int M, int N, int K, const void *d_A, const void *d_B, void *d_C) {
    if (K == 4096 && N == 4096) {
        Gemm_FP8_F32_LargeKN gemm_op;

        float alpha = 1.0f, beta = 0.0f;

        Gemm_FP8_F32_LargeKN::Arguments args({M, N, K}, {static_cast<const ElementA_f16 *>(d_A), K},
                                             {static_cast<const ElementB_f16 *>(d_B), K},
    ...
    Gemm_FP8_F32_Default::Arguments args({M, N, K}, {static_cast<const ElementA_f16 *>(d_A), K},
                                         {static_cast<const ElementB_f16 *>(d_B), K},
                                         {static_cast<ElementOut_f16 *>(d_C), N},
                                         {static_cast<ElementOut_f16 *>(d_C), N}, {alpha, beta});
...
int cutlass_fp8_bench(int M, int N, int K, int iters, int mode) {
    ...
    /* Warmup */
    int rc = (mode == 0) ? cutlass_fp8_gemm_f16acc(M, N, K, d_A, d_B, d_C)
